...
//            $("#medicalinfo-gender").val(data.gender == "MALE" ? "Nam" : "Nữ").trigger("change");
//            $("#medicalinfo-dob").val(moment(data.dob).format("DD/MM/YYYY"));
//            $("#medicalinfo-phone").val(data.phone.length > 0 ? data.phone : "");
//        }
//    });
//    //await LoadDataMedicalProfile();
//})
//Load data
...
$(document).ready(function () {
    ...
    $.when(LoadProfile()).done(async function () {
        if (profile.length > 0) {
            let data = profile[0];
            $("#medicalinfo-name").val(data.id > 0 ? data.name : "");
            $("#medicalinfo-gender").val(data.gender == "MALE" ? "Nam" : "Nữ").trigger("change");
                ...
                if (resultAccountSurvey.status == '200' && resultAccountSurvey.message == 'SUCCESS') {
                    if (resultAccountSurvey.data[0]) {
                        $("#medicalinfo_card_result").append(`
                            <div class="result-servey col-lg-6 ">
                                <a class="btn btnHS" href="/chi-tiet-bai-trac-nhiem-sktt?id=`+ resultAccountSurvey.data[0].id + `">Xem lại kết quả trắc nghiệm SKTT gần nhất</a>
                            </div> 
                            <div class="col-lg-6 mt-4">
                                <button id="survey-button" type="button" class="btn btnHS">
                                    Tham gia lại trắc nghiệm STKK
                                </button>
                            </div>
                            `)
                    }
    ...
    //2.Load Thể trạng
    $.when(LoadMedicalProfile()).done(function () {
        //debugger;
...
$(document).on("click", "#survey-button", function () {
    window.location.href = "/trac-nghiem-suc-khoe-tam-than"
})

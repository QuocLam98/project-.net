...
const load = async () => {
    ...
    if (bookingDetailId > 0) {
        ...
        if (booking_detail.status == "200") {
            var nameAddress = dataDetail.bookingTypeId == systemConstant.bookingType_online ? 'Số điện thoại Zalo (dành buổi tư vấn Online)' : 'Địa chỉ tư vấn';
...
const bookingCancel = async () => {
    ...
    var updatingObj = {
        ...
        "url": parseInt(bookingType) == systemConstant.bookingType_online ? address : '',
        "createdTime": createdTime,
        "reason": "Người dùng " + nameAccount + " đã hủy vì: " + $("#reason_cancel").val()
    };
    //console.log(updatingObj)
    Swal.fire({
        title: 'Đặt lịch tư vấn',

...
async function deleteItem(id) {
    ...
    swal.fire({
        ...
        html: 'Bạn có chắc chắn muốn xóa nhà tuyền dụng có thể bạn quan tâm <b>' + item.jobName + '</b>?',
        icon: 'warning',
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.isSucceeded) {
                        Swal.fire(
                            'Xóa nhà tuyền dụng có thể bạn quan tâm',
                            'Nhà tuyền dụng có thể bạn quan tâm <b>' + item.jobName + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        reGenTable();
                    }
...
function loadDataJobSelect() {
    //debugger;
    $.ajax({
        url: systemConfig.defaultAPIURL + "api/must-interesterd-company/list-job-selected",
        type: "GET",
        contentType: "application/json",
        success: function (response) {
            if (response.isSucceeded) {

...
$(document).ready(function () {
    ...
    $("#form-submit-interviewStatus").on("submit", function (e) {
        e.preventDefault();
        validate();
    })
...
async function editItem(id) {
    ...
    autosize.destroy($('#interviewStatusDescription'));
    autosize($('#interviewStatusDescription'));
    ...
    if (id > 0) {
        ...
        if (updatingObj != null && updatingObj != undefined) {
            ...
            $("#interviewStatusName").val(updatingObj.name).trigger("change");
            $("#interviewStatusDescription").val(updatingObj.description).trigger("change");
            $("#interviewStatusColor").val(updatingObj.color).trigger("change");
            ...
            swal.fire(
                ...
                'Không thể cập nhật trạng thái phỏng vấn, hãy kiểm tra lại thông tin.',
                'error'
            );
        }
        //var strLengthDes = $("#interviewStatusDescription").val().split('\n').length;
        //$("#interviewStatusDescription").attr('rows', strLengthDes < 2 ? 2 : strLengthDes);

    } else {
        $("#interviewStatusName").val("").trigger("change");
        $("#interviewStatusDescription").val("").trigger("change");
        $("#interviewStatusColor").val("").trigger("change");
    ...
    $("#interviewStatusModalTitle").text(id > 0 ? "Cập nhật trạng thái phỏng vấn" : "Thêm mới trạng thái phỏng vấn");
...
async function getItemById(id) {
    $("#loading").addClass("show");
    await $.ajax({
        url: systemConfig.defaultAPIURL + "api/interview-status/detail/" + id,
        type: "GET",
...
function loadData() {
    $.ajax({
        url: systemConfig.defaultAPIURL + "api/interview-status/list",
        type: "GET",
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa trạng thái phỏng vấn',
        html: 'Bạn có chắc chắn muốn xóa trạng thái phỏng vấn <b>' + updatingObj.name + '</b>?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $("#loading").addClass("show");
            $.ajax({
                url: systemConfig.defaultAPIURL + "api/interview-status/delete?id=" + id,
                type: "DELETE",
                success: function (response) {
                    $("#loading").removeClass("show");
                    if (response.isSucceeded) {
                        Swal.fire(
                            'Xóa trạng thái phỏng vấn',
                            'Trạng thái phỏng vấn <b>' + updatingObj.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        ...
                        if (response.status == 400) {
                            if (response.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Trạng thái phỏng vấn <p class="swal__admin__subtitle"> Xóa không thành công </p>',
                                    contentError,
                                    'warning'
                                );
                            } else {
                ...
                error: function (e) {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Xóa trạng thái phỏng vấn',
                            'Xóa trạng thái phỏng vấn không thành công, <br> vui lòng thử lại sau!',
                            'error'
                        );
                    }
...
function validate() {
    var errorList = [];
    if ($("#interviewStatusName").val().length == 0) {
        errorList.push("Tên không được bỏ trống.");
    } else if ($("#interviewStatusName").val().length > 255) {
        errorList.push("Tên không được dài quá 255 ký tự.");
    }
    if ($("#interviewStatusDescription").val().length > 500) {
        errorList.push("Mô tả không được dài quá 500 ký tự.");
    }
    if ($("#interviewStatusColor").val().length == 0) {
        errorList.push("Màu không được bỏ trống.");

    } else if ($("#interviewStatusColor").val().length > 12) {
    ...
    if (errorList.length > 0) {
        ...
        Swal.fire(
            'Trạng thái phỏng vấn' + swalSubTitle,
            contentError,
            'warning'
        );
    } else {
...
function submit() {
    var obj = {
        name: ($("#interviewStatusName").val() != '' ? $("#interviewStatusName").val().trim() : ""),
        description: ($("#interviewStatusDescription").val() != '' ? $("#interviewStatusDescription").val() : ""),
        color: ($("#interviewStatusColor").val() != '' ? $("#interviewStatusColor").val() : ""),
    }
    ...
    swal.fire({
        title: actionName + " trạng thái phỏng vấn",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " trạng thái phỏng vấn <b>" + $("#interviewStatusName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingId > 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/interview-status/update",
                    type: "PUT",
                    contentType: "application/json",
                    ...
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                ...
                                'Trạng thái phỏng vấn <b>' + $("#interviewStatusName").val() + '</b> đã được cập nhật thành công.',
                                'success'
                            ).then((result) => {
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Trạng thái phỏng vấn <p class="swal__admin__subtitle"> Cập nhật không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
            ...
            if (updatingId == 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/interview-status/add",
                    type: "POST",
                    contentType: "application/json",
                    data: JSON.stringify(obj),
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Thêm mới trạng thái phỏng vấn',
                                'Trạng thái phỏng vấn <b>' + $("#interviewStatusName").val() + '</b> đã được thêm mới thành công.',
                                'success'
                            ).then(function () {
                                $("#interviewStatusModal").modal("hide");
                    ...
                    error: function (e) {
                        ...
                        if (e.status === 401) {
                            ...
                            Swal.fire(
                                'Quản lý trạng thái phỏng vấn',
                                'Không thể thêm mới trạng thái phỏng vấn, hãy kiểm tra lại thông tin.',
                                'error'
                            );
                        }
                    }
...
function tableSearch() {
    table.column(1).search($("#searchInterviewStatusName").val());
    table.column(3).search($("#searchInterviewStatusDescription").val());
    table.draw();
}
...
$("#interviewStatusModal").on('shown.bs.modal', function () {
    autosize.destroy($('#interviewStatusDescription'));
    autosize($('#interviewStatusDescription'));
})
$("#interviewStatusModal").on('hiden.bs.modal', function () {
    autosize.destroy($('#interviewStatusDescription'));
})
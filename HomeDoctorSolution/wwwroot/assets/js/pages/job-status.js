...
async function editItem(id) {
    if (id > 0) {
        try {
            let result = await httpService.getAsync(`api/job-status/detail/${id}`);
    ...
    $("#jobStatusModalTitle").text(id > 0 ? "Cập nhật trạng thái công việc" : "Thêm mới trạng thái công việc");
...
async function loadData() {
    try {
        let res = await httpService.getAsync("api/job-status/list");
        if (res.isSucceeded) {
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa trạng thái công việc',
        html: 'Bạn có chắc chắn muốn xóa trạng thái công việc <b>' + item.name + '</b>',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            try {
                let result = await httpService.deleteAsync(`api/job-status/delete/${id}`)
                if (result.isSucceeded) {
                    if (result.status == 200) {
                        Swal.fire(
                            'Trạng thái công việc',
                            'Trạng thái công việc <b>' + item.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        refreshTable();
                    }
                    ...
                    Swal.fire(
                        'Trạng thái công việc',
                        'Xóa trạng thái công việc không thành công, <b> vui lòng thử lại sau.',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
                ...
                if (e.status === 401) {
                    ...
                    Swal.fire(
                        'Trạng thái công việc',
                        'Xóa trạng thái công việc không thành công, <br> vui lòng thử lại sau!',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
...
function submit() {
    ...
    Swal.fire({
        title: actionName + "trạng thái công việc",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " trạng thái công việc <b>" + $("#jobStatusName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingObj) {
                try {
                    let result;
                    if (updatingObj.id > 0) {
                        result = await httpService.putAsync("api/job-status/update", updatingObj);
                    } else {
                        result = await httpService.postAsync("api/job-status/add", updatingObj);
                    }

                    if (result.isSucceeded) {
                        Swal.fire(
                            'Trạng thái công việc',
                            'Trạng thái công việc <b>' + updatingObj.name + `</b> đã được ${actionName.toLowerCase()} thành công.`,
                            'success'
                        ).then((result) => {
                        ...
                        if (result.status == 400) {
                            ...
                            if (result.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Trạng thái công việc' + swalSubTitle,
                                    contentError,
                                    'warning'
                                );
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Trạng thái công việc',
                            'Bạn không có trạng thái công việc sử dụng tính năng này.',
                            'error'
                        );
                    }
                    else {
                        swal.fire(
                            'Trạng thái công việc',
                            `${actionName} trạng thái công việc không thành công, <br> vui lòng thử lại sau!`,
                            'error'
                        );
                    }
                    $("#loading").removeClass("show");

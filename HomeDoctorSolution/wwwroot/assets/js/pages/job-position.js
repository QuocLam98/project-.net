...
$(document).ready(function () {
    ...
    $("#form-submit-job-position").on("submit", function (e) {
        e.preventDefault();
        validate();
    })
    ...
    $("#btnTableResetSearch").click(function () {
        $("#searchJobPositionName").val("");
        $("#searchJobPositionDescription").val("");
...
async function editItem(id) {
    ...
    autosize.destroy($('#jobPositionDescription'));
    autosize($('#jobPositionDescription'));
    ...
    if (id > 0) {
        ...
        if (updatingObj != null && updatingObj != undefined) {
            ...
            $("#jobPositionName").val(updatingObj.name).trigger("change");
            $("#jobPositionDescription").val(updatingObj.description).trigger("change");
            ...
            swal.fire(
                ...
                'không thể cập nhật vị trí công việc, hãy kiểm tra lại thông tin.',
                'error'
            );
        }
        //var strLengthDes = $("#jobPositionModalDescription").val().split('\n').length;
        //$("#jobPositionModalDescription").attr('rows', strLengthDes < 2 ? 2 : strLengthDes);

    } else {
        $("#jobPositionName").val("").trigger("change");
        $("#jobPositionDescription").val("").trigger("change");
        $("#jobPositionColor").val("").trigger("change");

        $("#createdTime").val(moment().format("DD/MM/YYYY HH:mm:ss"));
    }


    $("#jobPositionModalTitle").text(id > 0 ? "Cập nhật vị trí công việc" : "Thêm mới vị trí công việc");
...
async function getItemById(id) {
    return (await $.ajax({
        url: systemConfig.defaultAPIURL + "api/job-position/detail/" + id,
        type: "GET",
...
async function deleteItem(id) {
    ...
    swal.fire({
        ...
        html: 'Bạn có chắc chắn muốn xóa vị trí công việc <b>' + updatingObj.name + '</b>?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $("#loading").addClass("show");
            $.ajax({
                url: systemConfig.defaultAPIURL + "api/job-position/delete?id=" + id,
                type: "DELETE",
                success: function (response) {
                    $("#loading").removeClass("show");
                    if (response.isSucceeded) {
                        Swal.fire(
                            'Xóa loại vị trí công việc',
                            'Loại vị trí công việc <b>' + updatingObj.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        reGenTable();
                    }
                        ...
                        if (response.status == 400) {
                            if (response.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Vị trí công việc <p class="swal__admin__subtitle"> Xóa không thành công </p>',
                                    contentError,
                                    'warning'
                                );
                            } else {
                ...
                error: function (e) {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            ...
                            'Xóa vị trí công việc không thành công, <br> vui lòng thử lại sau!',
                            'error'
                        );
                    }
...
function validate() {
    var errorList = [];
    if ($("#jobPositionName").val().length == 0) {
        errorList.push("Tên không được bỏ trống.");
    } else if ($("#jobPositionName").val().length > 255) {
        errorList.push("Tên không được dài quá 255 ký tự.");
    }
    if ($("#jobPositionDescription").val().length > 500) {
    ...
    if (errorList.length > 0) {
        ...
        Swal.fire(
            'Loại vị trí công việc' + swalSubTitle,
            contentError,
            'warning'
        );
    } else {
...
function submit() {
    var obj = {
        name: ($("#jobPositionName").val() != '' ? $("#jobPositionName").val().trim() : ""),
        description: ($("#jobPositionDescription").val() != '' ? $("#jobPositionDescription").val() : ""),
    }
    ...
    swal.fire({
        title: actionName + " vị trí công việc",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " vị trí công việc <b>" + $("#jobPositionName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingId > 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/job-position/update",
                    type: "PUT",
                    contentType: "application/json",
                    ...
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                ...
                                'Vị trí công việc <b>' + $("#jobPositionName").val() + '</b> đã được cập nhật thành công.',
                                'success'
                            ).then((result) => {
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Vị trí công việc <p class="swal__admin__subtitle"> Cập nhật không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
            ...
            if (updatingId == 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/job-position/add",
                    type: "POST",
                    contentType: "application/json",
                    data: JSON.stringify(obj),
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                ...
                                'Loại vị trí công việc <b>' + $("#jobPositionName").val() + '</b> đã được thêm mới thành công.',
                                'success'
                            ).then(function () {
                                $("#jobPositionModal").modal("hide");
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Vị trí công việc <p class="swal__admin__subtitle"> Thêm mới không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
...
function tableSearch() {
    table.column(1).search($("#searchJobPositionName").val());
    table.column(2).search($("#searchJobPositionDescription").val());
    table.draw();
}

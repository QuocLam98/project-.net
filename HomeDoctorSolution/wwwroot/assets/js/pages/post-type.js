...
$(document).ready(function () {
    ...
    $("#form-submit-postType").on("submit", function (e) {
        e.preventDefault();
        validatepostType();
    })
...
async function editItem(id) {
    ...
    autosize.destroy($('#postTypeDescription'));
    autosize($('#postTypeDescription'));
    ...
    if (id > 0) {
        ...
        if (updatingObj != null && updatingObj != undefined) {
            ...
            $("#postTypeName").val(updatingObj.name).trigger("change");
            $("#postTypeDescription").val(updatingObj.description).trigger("change");
            $("#postTypeColor").val(updatingObj.color).trigger("change");
            ...
            swal.fire(
                'Loại bài viết',
                'không thể cập nhật loại bài viết, hãy kiểm tra lại thông tin.',
                'error'
            );
        }
        //var strLengthDes = $("#postTypeDescription").val().split('\n').length;
        //$("#postTypeDescription").attr('rows', strLengthDes < 2 ? 2 : strLengthDes);

    } else {
        $("#postTypeName").val("").trigger("change");
        $("#postTypeDescription").val("").trigger("change");
        $("#postTypeColor").val("").trigger("change");
    ...
    $("#postTypeModalTitle").text(id > 0 ? "Cập nhật loại bài viết" : "Thêm mới loại bài viết");
...
async function getItemById(id) {
    $("#loading").addClass("show");
    await $.ajax({
        url: systemConfig.defaultAPIURL + "api/post-type/detail/" + id,
        type: "GET",
...
async function deleteItem(id) {
    await getItemById(id);
    swal.fire({
        title: 'Xóa loại bài viết',
        html: 'Bạn có chắc chắn muốn xóa loại bài viết <b>' + updatingObj.name + '</b>?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $("#loading").addClass("show");
            $.ajax({
                url: systemConfig.defaultAPIURL + "api/post-type/delete?id=" + id,
                type: "DELETE",
                success: function (response) {
                    $("#loading").removeClass("show");
                    if (response.isSucceeded) {
                        Swal.fire(
                            'Xóa loại bài viết',
                            'Loại bài viết <b>' + updatingObj.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        ...
                        if (response.status == 400) {
                            if (response.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Loại bài viết <p class="swal__admin__subtitle"> Xóa không thành công </p>',
                                    contentError,
                                    'warning'
                                );
                            } else {
                ...
                error: function (e) {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Xóa loại bài viết',
                            'Xóa loại bài viết không thành công, <br> vui lòng thử lại sau!',
                            'error'
                        );
                    }
...
function validatepostType() {
    var errorList = [];
    if ($("#postTypeName").val().length == 0) {
        errorList.push("Tên không được bỏ trống.");
    } else if ($("#postTypeName").val().length > 255) {
        errorList.push("Tên không được dài quá 255 ký tự.");
    }
    if ($("#postTypeDescription").val().length > 500) {
...
function submit() {
    var obj = {
        name: ($("#postTypeName").val() != '' ? $("#postTypeName").val().trim() : ""),
        description: ($("#postTypeDescription").val() != '' ? $("#postTypeDescription").val() : ""),
    }
    ...
    swal.fire({
        title: actionName + " loại bài viết",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " loại bài viết <b>" + $("#postTypeName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingId > 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/post-type/update",
                    type: "PUT",
                    contentType: "application/json",
                    ...
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                ...
                                'Loại bài viết <b>' + $("#postTypeName").val() + '</b> đã được cập nhật thành công.',
                                'success'
                            ).then((result) => {
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Loại bài viết <p class="swal__admin__subtitle"> Cập nhật không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
            ...
            if (updatingId == 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/post-type/add",
                    type: "POST",
                    contentType: "application/json",
                    data: JSON.stringify(obj),
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                ...
                                'Loại bài viết <b>' + $("#postTypeName").val() + '</b> đã được thêm mới thành công.',
                                'success'
                            ).then(function () {
                                $("#postTypeModal").modal("hide");
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Loại bài viết <p class="swal__admin__subtitle"> Thêm mới không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
                    ...
                    error: function (e) {
                        ...
                        if (e.status === 401) {
                            ...
                            Swal.fire(
                                'Quản lý loại bài viết',
                                'Không thể thêm mới loại bài viết, hãy kiểm tra lại thông tin.',
                                'error'
                            );
                        }
                    }
...
function tableSearch() {
    table.column(1).search($("#searchPostTypeName").val());
    table.column(2).search($("#searchPostTypeDescription").val());
    table.draw();
}
...
$("#postTypeModal").on('shown.bs.modal', function () {
    autosize.destroy($('#postTypeDescription'));
    autosize($('#postTypeDescription'));
})
$("#postTypeModal").on('hiden.bs.modal', function () {
    autosize.destroy($('#postTypeDescription'));
})
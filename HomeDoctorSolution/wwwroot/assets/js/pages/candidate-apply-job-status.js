...
$(document).ready(async function () {
    ...
    $("#form-submit-job-status").on("submit", function (e) {
        e.preventDefault();
        validate();
    })
...
$("#jobStatusName").on("keypress", function (e) {
    if (e.keyCode == 13) {
...
async function editItem(id) {
    if (id > 0) {
        try {
            let result = await httpService.getAsync(`api/candidate-apply-job-status/detail/${id}`);
    ...
    $("#jobStatusName").val(id > 0 ? updatingObj.name : "").trigger("change");
    $("#jobStatusDescription").val(id > 0 ? updatingObj.description : "").trigger("change");
    $("#jobStatusColor").val(id > 0 ? updatingObj.color : "").trigger("change");
    ...
    $("#jobStatusModalTitle").text(id > 0 ? "Cập nhật trạng thái ứng viên ứng tuyến" : "Thêm mới trạng thái ứng viên ứng tuyến");
...
async function loadData() {
    try {
        let res = await httpService.getAsync("api/candidate-apply-job-status/list-all");
        if (res.isSucceeded) {
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa trạng thái ứng viên ứng tuyến',
        html: 'Bạn có chắc chắn muốn xóa trạng thái ứng viên ứng tuyến <b>' + item.name + '</b>',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            try {
                let result = await httpService.deleteAsync(`api/candidate-apply-job-status/delete/${id}`)
                if (result.isSucceeded) {
                    if (result.status == 200) {
                        Swal.fire(
                            'Trạng thái ứng viên ứng tuyến',
                            'Trạng thái ứng viên ứng tuyến <b>' + item.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        refreshTable();
                    }
                    ...
                    Swal.fire(
                        'Trạng thái ứng viên ứng tuyến',
                        'Xóa trạng thái ứng viên ứng tuyến không thành công, <b> vui lòng thử lại sau.',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
                ...
                if (e.status === 401) {
                    ...
                    Swal.fire(
                        'Trạng thái ứng viên ứng tuyến',
                        'Xóa trạng thái ứng viên ứng tuyến không thành công, <br> vui lòng thử lại sau!',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
...
function submit() {
    ...
    updatingObj.name = $("#jobStatusName").val().trim();
    updatingObj.description = $("#jobStatusDescription").val();
    updatingObj.color = $("#jobStatusColor").val();

    let actionName = updatingObj.id != undefined ? "Cập nhật" : "Thêm mới";

    Swal.fire({
        title: actionName + "trạng thái ứng viên ứng tuyến",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " trạng thái ứng viên ứng tuyến <b>" + $("#jobStatusName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingObj) {
                try {
                    let result;
                    if (updatingObj.id > 0) {
                        result = await httpService.putAsync("api/candidate-apply-job-status/update", updatingObj);
                    } else {
                        result = await httpService.postAsync("api/candidate-apply-job-status/add", updatingObj);
                    }

                    if (result.isSucceeded) {
                        Swal.fire(
                            'Trạng thái ứng viên ứng tuyến',
                            'Trạng thái ứng viên ứng tuyến <b>' + updatingObj.name + `</b> đã được ${actionName.toLowerCase()} thành công.`,
                            'success'
                        ).then((result) => {
                        ...
                        if (result.status == 400) {
                            ...
                            if (result.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Trạng thái ứng viên ứng tuyến' + swalSubTitle,
                                    contentError,
                                    'warning'
                                );
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Trạng thái ứng viên ứng tuyến',
                            'Bạn không có trạng thái ứng viên ứng tuyến sử dụng tính năng này.',
                            'error'
                        );
                    }
                    else {
                        swal.fire(
                            'Trạng thái ứng viên ứng tuyến',
                            `${actionName} trạng thái ứng viên ứng tuyến không thành công, <br> vui lòng thử lại sau!`,
                            'error'
                        );
                    }
                    $("#loading").removeClass("show");

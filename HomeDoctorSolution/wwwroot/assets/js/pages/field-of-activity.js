...
$(document).ready(async function () {
    ...
    $("#form-submit-fieldOfActivity").on("submit", function (e) {
        e.preventDefault();
        validate();
    })
...
$("#fieldOfActivityName").on("keypress", function (e) {
    if (e.keyCode == 13) {
...
async function editItem(id) {
    if (id > 0) {
        try {
            let result = await httpService.getAsync(`api/field-of-activity/detail/${id}`);
    ...
    $("#fieldOfActivityName").val(id > 0 ? updatingObj.name : "").trigger("change");
    $("#fieldOfActivityDescription").val(id > 0 ? updatingObj.description : "").trigger("change");
    ...
    $("#fieldOfActivityModalTitle").text(id > 0 ? "Cập nhật lĩnh vực hoạt động" : "Thêm mới lĩnh vực hoạt động");
    $("#loading").removeClass("show");
    $("#fieldOfActivityModal").modal("show");
}


async function loadData() {
    try {
        let res = await httpService.getAsync("api/field-of-activity/list");
        if (res.isSucceeded) {
...
async function deleteItem(id) {
    ...
    swal.fire({
        ...
        html: 'Bạn có chắc chắn muốn xóa lĩnh vực hoạt động <b>' + item.name + '</b>',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            try {
                let result = await httpService.deleteAsync(`api/field-of-activity/delete/${id}`)
                if (result.isSucceeded) {
                    if (result.status == 200) {
                        Swal.fire(
                            'Lĩnh vực hoạt động',
                            'Lĩnh vực hoạt động <b>' + item.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        refreshTable();
                    }
                    ...
                    Swal.fire(
                        'Lĩnh vực hoạt động',
                        'Xóa lĩnh vực hoạt động không thành công, <b> vui lòng thử lại sau.',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
                ...
                if (e.status === 401) {
                    ...
                    Swal.fire(
                        'Lĩnh vực hoạt động',
                        'Xóa lĩnh vực hoạt động không thành công, <br> vui lòng thử lại sau!',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
...
function submit() {
    ...
    updatingObj.name = $("#fieldOfActivityName").val().trim();
    updatingObj.description = $("#fieldOfActivityDescription").val();

    let actionName = updatingObj.id != undefined ? "Cập nhật" : "Thêm mới";

    Swal.fire({
        title: actionName + "lĩnh vực hoạt động",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " lĩnh vực hoạt động <b>" + $("#fieldOfActivityName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingObj) {
                try {
                    let result;
                    if (updatingObj.id > 0) {
                        result = await httpService.putAsync("api/field-of-activity/update", updatingObj);
                    } else {
                        result = await httpService.postAsync("api/field-of-activity/add", updatingObj);
                    }

                    if (result.isSucceeded) {
                        Swal.fire(
                            'Lĩnh vực hoạt động',
                            'Lĩnh vực hoạt động <b>' + updatingObj.name + `</b> đã được ${actionName.toLowerCase()} thành công.`,
                            'success'
                        ).then((result) => {
                            $("#fieldOfActivityModal").modal("hide");
                            refreshTable();
                        });
                    } else {
                        if (result.status == 400) {
                            ...
                            if (result.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Lĩnh vực hoạt động' + swalSubTitle,
                                    contentError,
                                    'warning'
                                );
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Lĩnh vực hoạt động',
                            'Bạn không có lĩnh vực hoạt động sử dụng tính năng này.',
                            'error'
                        );
                    }
                    else {
                        swal.fire(
                            'Lĩnh vực hoạt động',
                            `${actionName} lĩnh vực hoạt động không thành công, <br> vui lòng thử lại sau!`,
                            'error'
                        );
                    }
                    $("#loading").removeClass("show");
...
function validate(obj) {
    var errorList = [];
    //if ($("#fieldOfActivityName").val().length === 0) {
    //    errorList.push("Tên không được để trống.");
    //} else if ($("#fieldOfActivityName").val().length > 255) {
    //    errorList.push("Tên không được dài quá 255 ký tự.");
    //}

    //if ($("#fieldOfActivityValue").val().length === 0) {
    //    errorList.push("Giá trị không được để trống.");
    //} else if ($("#fieldOfActivityValue").val().length > 255) {
    //    errorList.push("Giá trị không được dài quá 255 ký tự.");
    //}

    //if ($("#fieldOfActivityDescription").val().length > 500) {

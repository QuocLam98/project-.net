...
$(document).ready(function () {
    ...
    $("#form-submit-salary-type").on("submit", function (e) {
        e.preventDefault();
        validateJobSkill();
    })
    ...
    $("#btnTableResetSearch").click(function () {
        $("#searchSalaryTypeName").val("");
        $("#searchSalaryTypeDescription").val("");
...
async function editItem(id) {
    ...
    autosize.destroy($('#salaryTypeDescription'));
    autosize($('#salaryTypeDescription'));
    ...
    if (id > 0) {
        ...
        if (updatingObj != null && updatingObj != undefined) {
            ...
            $("#salaryTypeName").val(updatingObj.name).trigger("change");
            $("#salaryTypeDescription").val(updatingObj.description).trigger("change");
            ...
            swal.fire(
                'Loại lương',
                'không thể cập nhật loại lương, hãy kiểm tra lại thông tin.',
                'error'
            );
        }
        //var strLengthDes = $("#salaryTypeModalDescription").val().split('\n').length;
        //$("#salaryTypeModalDescription").attr('rows', strLengthDes < 2 ? 2 : strLengthDes);

    } else {
        $("#salaryTypeName").val("").trigger("change");
        $("#salaryTypeDescription").val("").trigger("change");
        $("#salaryTypeColor").val("").trigger("change");

        $("#createdTime").val(moment().format("DD/MM/YYYY HH:mm:ss"));
    }


    $("#salaryTypeModalTitle").text(id > 0 ? "Cập nhật loại lương" : "Thêm mới loại lương");
    $("#loading").removeClass("show");
    $("#salaryTypeModal").modal("show");

}

async function getItemById(id) {
    //return (await $.ajax({
    //    url: systemConfig.defaultAPIURL + "api/salary-type/detail/"+id,
    //    type: "GET",
...
function loadData() {
    $.ajax({
        url: systemConfig.defaultAPIURL + "api/salary-type/list",
        type: "GET",
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa loại lương',
        html: 'Bạn có chắc chắn muốn xóa loại lương <b>' + updatingObj.name + '</b>?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $("#loading").addClass("show");
            $.ajax({
                url: systemConfig.defaultAPIURL + "api/salary-type/delete?id=" + id,
                type: "DELETE",
                success: function (response) {
                    $("#loading").removeClass("show");
                    if (response.isSucceeded) {
                        Swal.fire(
                            'Xóa loại loại lương',
                            'Loại lương <b>' + updatingObj.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        reGenTable();
                    }
                        ...
                        if (response.status == 400) {
                            if (response.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Loại lương <p class="swal__admin__subtitle"> Xóa không thành công </p>',
                                    contentError,
                                    'warning'
                                );
                            } else {
                ...
                error: function (e) {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Xóa loại lương',
                            'Xóa loại lương không thành công, <br> vui lòng thử lại sau!',
                            'error'
                        );
                    }
...
function validateJobSkill() {
    var errorList = [];
    if ($("#salaryTypeName").val().length == 0) {
        errorList.push("Tên không được bỏ trống.");
    } else if ($("#salaryTypeName").val().length > 255) {
        errorList.push("Tên không được dài quá 255 ký tự.");
    }
    if ($("#salaryTypeDescription").val().length > 500) {
...
function submit() {
    var obj = {
        name: ($("#salaryTypeName").val() != '' ? $("#salaryTypeName").val().trim() : ""),
        description: ($("#salaryTypeDescription").val() != '' ? $("#salaryTypeDescription").val() : ""),
    }
    ...
    swal.fire({
        title: actionName + " loại lương",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " loại lương <b>" + $("#salaryTypeName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingId > 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/salary-type/update",
                    type: "PUT",
                    contentType: "application/json",
                    ...
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Cập nhật loại lương',
                                'Loại lương <b>' + $("#salaryTypeName").val() + '</b> đã được cập nhật thành công.',
                                'success'
                            ).then((result) => {
                                $("#salaryTypeModal").modal("hide");
                                reGenTable();
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Loại lương <p class="swal__admin__subtitle"> Cập nhật không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
            ...
            if (updatingId == 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/salary-type/add",
                    type: "POST",
                    contentType: "application/json",
                    data: JSON.stringify(obj),
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Thêm mới loại lương',
                                'Loại lương <b>' + $("#salaryTypeName").val() + '</b> đã được thêm mới thành công.',
                                'success'
                            ).then(function () {
                                $("#salaryTypeModal").modal("hide");
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Loại lương <p class="swal__admin__subtitle"> Thêm mới không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
                    ...
                    error: function (e) {
                        ...
                        if (e.status === 401) {
                            ...
                            Swal.fire(
                                'Quản lý loại lương',
                                'Không thể thêm mới loại lương, hãy kiểm tra lại thông tin.',
                                'error'
                            );
                        }
                    }
...
function tableSearch() {
    table.column(1).search($("#searchSalaryTypeName").val());
    table.column(2).search($("#searchSalaryDescription").val());
    table.draw();
}

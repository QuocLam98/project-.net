...
$(document).ready(async function () {
    ...
    $("#form-submit-salaryrange").on("submit", function (e) {
        e.preventDefault();
        validate();
    })
...
$("#salaryrangeName").on("keypress", function (e) {
    if (e.keyCode == 13) {
...
async function editItem(id) {
    if (id > 0) {
        try {
            let result = await httpService.getAsync(`api/salary-range/detail/${id}`);
    ...
    $("#salaryrangeName").val(id > 0 ? updatingObj.name : "").trigger("change");
    $("#salaryrangeDescription").val(id > 0 ? updatingObj.description : "").trigger("change");
    ...
    $("#salaryrangeModalTitle").text(id > 0 ? "Cập nhật mức lương" : "Thêm mới mức lương");
    $("#loading").removeClass("show");
    $("#salaryrangeModal").modal("show");
}
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa mức lương',
        html: 'Bạn có chắc chắn muốn xóa mức lương <b>' + item.name + '</b>',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            try {
                let result = await httpService.deleteAsync(`api/salary-range/delete/${id}`)
                if (result.isSucceeded) {
                    if (result.status == 200) {
                        Swal.fire(
                            'Mức lương',
                            'Mức lương <b>' + item.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        refreshTable();
                    }
                    ...
                    Swal.fire(
                        'Mức lương',
                        'Xóa mức lương không thành công, <b> vui lòng thử lại sau.',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
                ...
                if (e.status === 401) {
                    ...
                    Swal.fire(
                        'Mức lương',
                        'Xóa mức lương không thành công, <br> vui lòng thử lại sau!',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
...
function submit() {
    ...
    updatingObj.name = $("#salaryrangeName").val().trim();
    updatingObj.description = $("#salaryrangeDescription").val();

    let actionName = updatingObj.id != undefined ? "Cập nhật" : "Thêm mới";

    Swal.fire({
        title: actionName + "mức lương",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " mức lương <b>" + $("#salaryrangeName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingObj) {
                try {
                    let result;
                    if (updatingObj.id > 0) {
                        result = await httpService.putAsync("api/salary-range/update", updatingObj);
                    } else {
                        result = await httpService.postAsync("api/salary-range/add", updatingObj);
                    }

                    if (result.isSucceeded) {
                        Swal.fire(
                            'Mức lương',
                            'Mức lương <b>' + updatingObj.name + `</b> đã được ${actionName.toLowerCase()} thành công.`,
                            'success'
                        ).then((result) => {
                            $("#salaryrangeModal").modal("hide");
                            refreshTable();
                        });
                    } else {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Mức lương',
                            'Bạn không có mức lương sử dụng tính năng này.',
                            'error'
                        );
                    }
                    else {
                        swal.fire(
                            'Mức lương',
                            `${actionName} mức lương không thành công, <br> vui lòng thử lại sau!`,
                            'error'
                        );
                    }
                    $("#loading").removeClass("show");
...
function validate(obj) {
    var errorList = [];
    //if ($("#salaryrangeName").val().length === 0) {
    //    errorList.push("Tên không được để trống.");
    //} else if ($("#salaryrangeName").val().length > 255) {
    //    errorList.push("Tên không được dài quá 255 ký tự.");
    //}

    //if ($("#salaryrangeValue").val().length === 0) {
    //    errorList.push("Giá trị không được để trống.");
    //} else if ($("#salaryrangeValue").val().length > 255) {
    //    errorList.push("Giá trị không được dài quá 255 ký tự.");
    //}

    //if ($("#salaryrangeDescription").val().length > 500) {

...
$(document).ready(async function () {
    ...
    $("#form-submit-interviewType").on("submit", function (e) {
        e.preventDefault();
        validate();
    })
...
$("#interviewTypeName").on("keypress", function (e) {
    if (e.keyCode == 13) {
...
async function editItem(id) {
    if (id > 0) {
        try {
            let result = await httpService.getAsync(`api/interview-type/detail/${id}`);
    ...
    $("#interviewTypeName").val(id > 0 ? updatingObj.name : "").trigger("change");
    $("#interviewTypeDescription").val(id > 0 ? updatingObj.description : "").trigger("change");
    ...
    $("#interviewTypeModalTitle").text(id > 0 ? "Cập nhật loại phỏng vấn" : "Thêm mới loại phỏng vấn");
    $("#loading").removeClass("show");
    $("#interviewTypeModal").modal("show");
}


async function loadData() {
    try {
        let res = await httpService.getAsync("api/interview-type/list");
        if (res.isSucceeded) {
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa loại phỏng vấn',
        html: 'Bạn có chắc chắn muốn xóa loại phỏng vấn <b>' + item.name + '</b>',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            try {
                let result = await httpService.deleteAsync(`api/interview-type/delete/${id}`)
                if (result.isSucceeded) {
                    if (result.status == 200) {
                        Swal.fire(
                            'Loại phỏng vấn',
                            'Loại phỏng vấn <b>' + item.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        refreshTable();
                    }
                    ...
                    Swal.fire(
                        'Loại phỏng vấn',
                        'Xóa loại phỏng vấn không thành công, <b> vui lòng thử lại sau.',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
                ...
                if (e.status === 401) {
                    ...
                    Swal.fire(
                        'Loại phỏng vấn',
                        'Xóa loại phỏng vấn không thành công, <br> vui lòng thử lại sau!',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
...
function submit() {
    ...
    updatingObj.name = $("#interviewTypeName").val().trim();
    updatingObj.description = $("#interviewTypeDescription").val();

    let actionName = updatingObj.id != undefined ? "Cập nhật" : "Thêm mới";

    Swal.fire({
        title: actionName + "loại phỏng vấn",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " loại phỏng vấn <b>" + $("#interviewTypeName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingObj) {
                try {
                    let result;
                    if (updatingObj.id > 0) {
                        result = await httpService.putAsync("api/interview-type/update", updatingObj);
                    } else {
                        result = await httpService.postAsync("api/interview-type/add", updatingObj);
                    }

                    if (result.isSucceeded) {
                        Swal.fire(
                            'Loại phỏng vấn',
                            'Loại phỏng vấn <b>' + updatingObj.name + `</b> đã được ${actionName.toLowerCase()} thành công.`,
                            'success'
                        ).then((result) => {
                            $("#interviewTypeModal").modal("hide");
                            refreshTable();
                        });
                    } else {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Loại phỏng vấn',
                            'Bạn không có loại phỏng vấn sử dụng tính năng này.',
                            'error'
                        );
                    }
                    else {
                        swal.fire(
                            'Loại phỏng vấn',
                            `${actionName} loại phỏng vấn không thành công, <br> vui lòng thử lại sau!`,
                            'error'
                        );
                    }
                    $("#loading").removeClass("show");
...
function validate(obj) {
    var errorList = [];
    //if ($("#interviewTypeName").val().length === 0) {
    //    errorList.push("Tên không được để trống.");
    //} else if ($("#interviewTypeName").val().length > 255) {
    //    errorList.push("Tên không được dài quá 255 ký tự.");
    //}

    //if ($("#interviewTypeValue").val().length === 0) {
    //    errorList.push("Giá trị không được để trống.");
    //} else if ($("#interviewTypeValue").val().length > 255) {
    //    errorList.push("Giá trị không được dài quá 255 ký tự.");
    //}

    //if ($("#interviewTypeDescription").val().length > 500) {

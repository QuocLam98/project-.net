...
$(document).ready(function () {
    ...
    $("#form-submit-menu-type").on("submit", function (e) {
        e.preventDefault();
        validateJobSkill();
    })
    ...
    $("#btnTableResetSearch").click(function () {
        $("#searchMenuName").val("");
        $("#searchMenuType").val("").trigger("change");
        $("#searchMenuLink").val("");
...
async function editItem(id) {
    ...
    if (id > 0) {
        ...
        if (updatingObj != null && updatingObj != undefined) {
            ...
            $("#menuHomepageName").val(updatingObj.name).trigger("change");
            $("#selectMenuType").val(updatingObj.menuTypeId).trigger("change");
            $("#menuHomepageLink").val(updatingObj.link).trigger("change");
            $("#menuIcon").val(updatingObj.icon).trigger("change");
            $("#createdTime").val(moment(updatingObj.createdTime).format("DD/MM/YYYY HH:mm:ss"));
        }
        else {
            swal.fire(
                'Loại menu',
                'không thể cập nhật loại menu, hãy kiểm tra lại thông tin.',
                'error'
            );
        }
        //var strLengthDes = $("#menuHomePageModalDescription").val().split('\n').length;
        //$("#menuHomePageModalDescription").attr('rows', strLengthDes < 2 ? 2 : strLengthDes);

    } else {
        $("#menuHomepageName").val("").trigger("change");
        $("#selectMenuType").val(DEFAULT_VALUE_MENU_ADMIN).trigger("change");
        $("#menuHomepageLink").val("").trigger("change");
        $("#menuIcon").val("").trigger("change");
        $("#createdTime").val(moment().format("DD/MM/YYYY HH:mm:ss"));
    }


    $("#menuHomePageModalTitle").text(id > 0 ? "Cập nhật loại menu" : "Thêm mới loại menu");
    $("#loading").removeClass("show");
    $("#menuHomePageModal").modal("show");

}

async function getItemById(id) {
    await $.ajax({
        url: systemConfig.defaultAPIURL + "api/menu/detail/" + id,
        type: "GET",
...
function loadData() {
    $.ajax({
        url: systemConfig.defaultAPIURL + "api/menu/list-menu",
        type: "GET",
...
function loadMenuType() {
    $.ajax({
        url: systemConfig.defaultAPIURL + "api/menu-type/list",
        type: "GET",
        contentType: "application/json",
        success: function (response) {
            ...
            data.forEach(function (item, index) {
                ...
                $("#searchMenuType").append(new Option(data.text, data.id, false, false)).trigger('change');
                $("#selectMenuType").append(new Option(data.text, data.id, false, false)).trigger('change');
...
function loadTable() {
    ...
    dataSource.forEach(function (item, index) {
        ...
        rowContent += "<td>" + "<span style='display : none;'>" + (item.menuTypeId != null ? item.menuTypeId : "") + "</span>" + (item.menuTypeName != null ? item.menuTypeName : "") + "</td>";
        rowContent += "<td>" + (item.link != null ? item.link : "") + "</td>";
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa loại menu',
        html: 'Bạn có chắc chắn muốn xóa loại menu <b>' + updatingObj.name + '</b>?',
        icon: 'warning',
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.isSucceeded) {
                        Swal.fire(
                            'Xóa loại loại menu',
                            'Loại menu <b>' + updatingObj.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        reGenTable();
                    }
                        ...
                        if (response.status == 400) {
                            if (response.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Loại menu <p class="swal__admin__subtitle"> Xóa không thành công </p>',
                                    contentError,
                                    'warning'
                                );
                            } else {
                ...
                error: function (e) {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Xóa loại menu',
                            'Xóa loại menu không thành công, <br> vui lòng thử lại sau!',
                            'error'
                        );
                    }
...
function submit() {
    var obj = {
        name: ($("#menuHomepageName").val() != '' ? $("#menuHomepageName").val().trim() : ""),
        link: ($("#menuHomepageLink").val() != '' ? $("#menuHomepageLink").val().trim() : ""),
        menuTypeId: ($("#selectMenuType").val() != '' ? $("#selectMenuType").val().trim() : ""),
        icon: ($("#menuIcon").val() != '' ? $("#menuIcon").val().trim() : "")
    }
    ...
    swal.fire({
        title: actionName + "menu",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + "menu <b>" + $("#menuHomepageName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Cập nhật loại menu',
                                'Loại menu <b>' + $("#menuHomepageName").val() + '</b> đã được cập nhật thành công.',
                                'success'
                            ).then((result) => {
                                $("#menuHomePageModal").modal("hide");
                                reGenTable();
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Loại menu <p class="swal__admin__subtitle"> Cập nhật không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Thêm mới loại menu',
                                'Loại menu <b>' + $("#menuHomepageName").val() + '</b> đã được thêm mới thành công.',
                                'success'
                            ).then(function () {
                                $("#menuHomePageModal").modal("hide");
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Loại menu <p class="swal__admin__subtitle"> Thêm mới không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
                    ...
                    error: function (e) {
                        ...
                        if (e.status === 401) {
                            ...
                            Swal.fire(
                                'Quản lý loại menu',
                                'Không thể thêm mới loại menu, hãy kiểm tra lại thông tin.',
                                'error'
                            );
                        }
                    }
...
function tableSearch() {
    table.column(1).search($("#searchMenuName").val());
    table.column(2).search($("#searchMenuType").val());
    table.column(3).search($("#searchMenuLink").val());
    table.draw();
}

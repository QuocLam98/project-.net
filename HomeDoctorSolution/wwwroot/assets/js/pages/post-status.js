...
$(document).ready(function () {
    ...
    $("#form-submit-postStatus").on("submit", function (e) {
        e.preventDefault();
        validatePostStatus();
    })
...
async function editItem(id) {
    ...
    autosize.destroy($('#postStatusDescription'));
    autosize($('#postStatusDescription'));
    ...
    if (id > 0) {
        ...
        if (updatingObj != null && updatingObj != undefined) {
            ...
            $("#postStatusName").val(updatingObj.name).trigger("change");
            $("#postStatusDescription").val(updatingObj.description).trigger("change");
            $("#postStatusColor").val(updatingObj.color).trigger("change");
            ...
            swal.fire(
                'Trạng thái bài viết',
                'Không thể cập nhật trạng thái bài viết, hãy kiểm tra lại thông tin.',
                'error'
            );
        }
        //var strLengthDes = $("#postStatusDescription").val().split('\n').length;
        //$("#postStatusDescription").attr('rows', strLengthDes < 2 ? 2 : strLengthDes);

    } else {
        $("#postStatusName").val("").trigger("change");
        $("#postStatusDescription").val("").trigger("change");
        $("#postStatusColor").val("").trigger("change");
    ...
    $("#postStatusModalTitle").text(id > 0 ? "Cập nhật trạng thái bài viết" : "Thêm mới trạng thái bài viết");
...
async function getItemById(id) {
    $("#loading").addClass("show");
    await $.ajax({
        url: systemConfig.defaultAPIURL + "api/post-status/detail/" + id,
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa trạng thái bài viết',
        html: 'Bạn có chắc chắn muốn xóa trạng thái bài viết <b>' + updatingObj.name + '</b>?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $("#loading").addClass("show");
            $.ajax({
                url: systemConfig.defaultAPIURL + "api/post-status/delete?id=" + id,
                type: "DELETE",
                success: function (response) {
                    $("#loading").removeClass("show");
                    if (response.isSucceeded) {
                        Swal.fire(
                            'Xóa trạng thái bài viết',
                            'Trạng thái bài viết <b>' + updatingObj.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        ...
                        if (response.status == 400) {
                            if (response.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Trạng thái bài viết <p class="swal__admin__subtitle"> Xóa không thành công </p>',
                                    contentError,
                                    'warning'
                                );
                            } else {
                ...
                error: function (e) {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Xóa trạng thái bài viết',
                            'Xóa trạng thái bài viết không thành công, <br> vui lòng thử lại sau!',
                            'error'
                        );
                    }
...
function validatePostStatus() {
    var errorList = [];
    if ($("#postStatusName").val().length == 0) {
        errorList.push("Tên không được bỏ trống.");
    } else if ($("#postStatusName").val().length > 255) {
        errorList.push("Tên không được dài quá 255 ký tự.");
    }
    if ($("#postStatusDescription").val().length >500) { 
    ...
    if (errorList.length > 0) {
        ...
        Swal.fire(
            'Trạng thái bài viết' + swalSubTitle,
            contentError,
            'warning'
        );
    } else {
...
function submit() {
    var obj = {
        name: ($("#postStatusName").val() != '' ? $("#postStatusName").val().trim() : ""),
        description: ($("#postStatusDescription").val() != '' ? $("#postStatusDescription").val() : ""),
        color: ($("#postStatusColor").val() != '' ? $("#postStatusColor").val() : ""),
    }
    ...
    swal.fire({
        title: actionName + " trạng thái bài viết",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " trạng thái bài viết <b>" + $("#postStatusName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingId > 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/post-status/update",
                    type: "PUT",
                    contentType: "application/json",
                    ...
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                ...
                                'Trạng thái bài viết <b>' + $("#postStatusName").val() + '</b> đã được cập nhật thành công.',
                                'success'
                            ).then((result) => {
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Trạng thái bài viết <p class="swal__admin__subtitle"> Cập nhật không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
            ...
            if (updatingId == 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/post-status/add",
                    type: "POST",
                    contentType: "application/json",
                    data: JSON.stringify(obj),
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Thêm mới trạng thái bài viết',
                                'Trạng thái bài viết <b>' + $("#postStatusName").val() + '</b> đã được thêm mới thành công.',
                                'success'
                            ).then(function () {
                                $("#postStatusModal").modal("hide");
                    ...
                    error: function (e) {
                        ...
                        if (e.status === 401) {
                            ...
                            Swal.fire(
                                'Quản lý trạng thái bài viết',
                                'Không thể thêm mới trạng thái bài viết, hãy kiểm tra lại thông tin.',
                                'error'
                            );
                        }
                    }
...
function tableSearch() {
    table.column(1).search($("#searchPostStatusName").val());
    table.column(3).search($("#searchPostStatusDescription").val());
    table.draw();
}
...
$("#postStatusModal").on('hiden.bs.modal', function () {
    autosize.destroy($('#postStatusDescription'));
})
...
$(document).ready(async function () {
    ...
    $("#form-submit-tagType").on("submit", function (e) {
        e.preventDefault();
        validate();
    })
...
$("#tagTypeName").on("keypress", function (e) {
    if (e.keyCode == 13) {
...
async function editItem(id) {
    if (id > 0) {
        try {
            let result = await httpService.getAsync(`api/tag-type/detail/${id}`);
    ...
    $("#tagTypeName").val(id > 0 ? updatingObj.name : "").trigger("change");
    $("#tagTypeDescription").val(id > 0 ? updatingObj.description : "").trigger("change");
    ...
    $("#tagTypeModalTitle").text(id > 0 ? "Cập nhật loại tag" : "Thêm mới loại tag");
...
async function loadData() {
    try {
        let res = await httpService.getAsync("api/tag-type/list");
        if (res.isSucceeded) {
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa loại tag',
        html: 'Bạn có chắc chắn muốn xóa loại tag <b>' + item.name + '</b>',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            try {
                let result = await httpService.deleteAsync(`api/tag-type/delete/${id}`)
                if (result.isSucceeded) {
                    if (result.status == 200) {
                        Swal.fire(
                            'Loại tag',
                            'Loại tag <b>' + item.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        refreshTable();
                    }
                    ...
                    Swal.fire(
                        'Loại tag',
                        'Xóa loại tag không thành công, <b> vui lòng thử lại sau.',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
                ...
                if (e.status === 401) {
                    ...
                    Swal.fire(
                        'Loại tag',
                        'Xóa loại tag không thành công, <br> vui lòng thử lại sau!',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
...
function submit() {
    ...
    updatingObj.name = $("#tagTypeName").val().trim();
    updatingObj.description = $("#tagTypeDescription").val();

    let actionName = updatingObj.id != undefined ? "Cập nhật" : "Thêm mới";

    Swal.fire({
        title: actionName + "loại tag",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " loại tag <b>" + $("#tagTypeName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingObj) {
                try {
                    let result;
                    if (updatingObj.id > 0) {
                        result = await httpService.putAsync("api/tag-type/update", updatingObj);
                    } else {
                        result = await httpService.postAsync("api/tag-type/add", updatingObj);
                    }

                    if (result.isSucceeded) {
                        Swal.fire(
                            'Loại tag',
                            'Loại tag <b>' + updatingObj.name + `</b> đã được ${actionName.toLowerCase()} thành công.`,
                            'success'
                        ).then((result) => {
                            $("#tagTypeModal").modal("hide");
                            refreshTable();
                        });
                    } else {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Loại tag',
                            'Bạn không có loại tag sử dụng tính năng này.',
                            'error'
                        );
                    }
                    else {
                        swal.fire(
                            'Loại tag',
                            `${actionName} loại tag không thành công, <br> vui lòng thử lại sau!`,
                            'error'
                        );
                    }
                    $("#loading").removeClass("show");

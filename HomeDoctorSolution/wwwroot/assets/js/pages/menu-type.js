...
$(document).ready(function () {
    ...
    $("#btnTableResetSearch").click(function () {
        $("#searchmenuTypeName").val("");
        $("#searchmenuTypeDescription").val("");
...
async function editItem(id) {
    ...
    autosize.destroy($('#menuTypeDescription'));
    autosize($('#menuTypeDescription'));
    ...
    if (id > 0) {
        ...
        if (updatingObj != null && updatingObj != undefined) {
            ...
            $("#menuTypeName").val(updatingObj.name).trigger("change");
            $("#menuTypeDescription").val(updatingObj.description).trigger("change");
        ...
        $("#menuTypeName").val("").trigger("change");
        $("#menuTypeDescription").val("").trigger("change");
        $("#menuTypeColor").val("").trigger("change");

        $("#createdTime").val(moment().format("DD/MM/YYYY HH:mm:ss"));
    }


    $("#menuTypeModalTitle").text(id > 0 ? "Cập nhật loại menu" : "Thêm mới loại menu");
    $("#loading").removeClass("show");
    $("#menuTypeModal").modal("show");

}

async function getItemById(id) {
    await $.ajax({
        url: systemConfig.defaultAPIURL + "api/menu-type/detail/" + id,
        type: "GET",
...
async function deleteItem(id) {
    ...
    swal.fire({
        ...
        if (result.isConfirmed) {
            $("#loading").addClass("show");
            $.ajax({
                url: systemConfig.defaultAPIURL + "api/menu-type/delete?id=" + id,
                type: "DELETE",
                success: function (response) {
                    $("#loading").removeClass("show");
                    if (response.isSucceeded) {
...
function validateJobSkill() {
    var errorList = [];
    //if ($("#menuTypeName").val().length == 0) {
    //    errorList.push("Tên không được bỏ trống.");
    //} else if ($("#menuTypeName").val().length > 255) {
    //    errorList.push("Tên không được dài quá 255 ký tự.");
    //}
    //if ($("#menuTypeDescription").val().length > 500) {
...
function submit() {
    var obj = {
        name: ($("#menuTypeName").val() != '' ? $("#menuTypeName").val().trim() : ""),
        description: ($("#menuTypeDescription").val() != '' ? $("#menuTypeDescription").val() : ""),
    }
    ...
    swal.fire({
        title: actionName + " loại menu",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " loại menu <b>" + $("#menuTypeName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingId > 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/menu-type/update",
                    type: "PUT",
                    contentType: "application/json",
                    ...
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Cập nhật loại menu',
                                'Loại menu <b>' + $("#menuTypeName").val() + '</b> đã được cập nhật thành công.',
                                'success'
                            ).then((result) => {
            ...
            if (updatingId == 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/menu-type/add",
                    type: "POST",
                    contentType: "application/json",
                    data: JSON.stringify(obj),
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Thêm mới loại menu',
                                'Loại menu <b>' + $("#menuTypeName").val() + '</b> đã được thêm mới thành công.',
                                'success'
                            ).then(function () {
                                $("#menuTypeModal").modal("hide");
...
function tableSearch() {
    table.column(1).search($("#searchmenuTypeName").val());
    table.column(2).search($("#searchmenuDescription").val());
    table.draw();
}

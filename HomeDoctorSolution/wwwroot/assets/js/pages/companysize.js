...
$(document).ready(async function () {
    ...
    $("#form-submit-company-size").on("submit", function (e) {
        e.preventDefault();
        validate();
    })
...
$("#companySizeName").on("keypress", function (e) {
    if (e.keyCode == 13) {
...
async function editItem(id) {
    if (id > 0) {
        try {
            let result = await httpService.getAsync(`api/company-size/detail/${id}`);
    ...
    $("#companySizeName").val(id > 0 ? updatingObj.name : "").trigger("change");
    $("#companySizeDescription").val(id > 0 ? updatingObj.description : "").trigger("change");
    ...
    $("#companySizeModalTitle").text(id > 0 ? "Cập nhật quy mô công ty" : "Thêm mới quy mô công ty");
    $("#loading").removeClass("show");
    $("#companySizeModal").modal("show");
}
...
async function deleteItem(id) {
    ...
    swal.fire({
        ...
        html: 'Bạn có chắc chắn muốn xóa quy mô công ty <b>' + item.name + '</b>',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            try {
                let result = await httpService.deleteAsync(`api/company-size/delete/${id}`)
                if (result.isSucceeded) {
                    if (result.status == 200) {
                        Swal.fire(
                            'Quy mô công ty',
                            'Quy mô công ty <b>' + item.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        refreshTable();
                    }
                    ...
                    Swal.fire(
                        'Quy mô công ty',
                        'Xóa quy mô công ty không thành công, <b> vui lòng thử lại sau.',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
                ...
                if (e.status === 401) {
                    ...
                    Swal.fire(
                        'Quy mô công ty',
                        'Xóa quy mô công ty không thành công, <br> vui lòng thử lại sau!',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
...
function submit() {
    ...
    updatingObj.name = $("#companySizeName").val().trim();
    updatingObj.description = $("#companySizeDescription").val();

    let actionName = updatingObj.id != undefined ? "Cập nhật" : "Thêm mới";

    Swal.fire({
        title: actionName + "quy mô công ty",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " quy mô công ty <b>" + $("#companySizeName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingObj) {
                try {
                    let result;
                    if (updatingObj.id > 0) {
                        result = await httpService.putAsync("api/company-size/update", updatingObj);
                    } else {
                        result = await httpService.postAsync("api/company-size/add", updatingObj);
                    }

                    if (result.isSucceeded) {
                        Swal.fire(
                            'Quy mô công ty',
                            'Quy mô công ty <b>' + updatingObj.name + `</b> đã được ${actionName.toLowerCase()} thành công.`,
                            'success'
                        ).then((result) => {
                            $("#companySizeModal").modal("hide");
                            refreshTable();
                        });
                    } else {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Quy mô công ty',
                            'Bạn không có quy mô công ty sử dụng tính năng này.',
                            'error'
                        );
                    }
                    else {
                        swal.fire(
                            'Quy mô công ty',
                            `${actionName} quy mô công ty không thành công, <br> vui lòng thử lại sau!`,
                            'error'
                        );
                    }
                    $("#loading").removeClass("show");
...
function validate(obj) {
    var errorList = [];
    //if ($("#companySizeName").val().length === 0) {
    //    errorList.push("Tên không được để trống.");
    //} else if ($("#companySizeName").val().length > 255) {
    //    errorList.push("Tên không được dài quá 255 ký tự.");
    //}

    //if ($("#companySizeValue").val().length === 0) {
    //    errorList.push("Giá trị không được để trống.");
    //} else if ($("#companySizeValue").val().length > 255) {
    //    errorList.push("Giá trị không được dài quá 255 ký tự.");
    //}

    //if ($("#companySizeDescription").val().length > 500) {

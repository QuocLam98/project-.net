...
$(document).ready(function () {
    ...
    $("#form-submit-top-job-urgent").on("submit", function (e) {
        e.preventDefault();
        validateJobSkill();
    })
...
async function editItem(id) {
    ...
    if (id > 0) {
        ...
        if (updatingObj != null && updatingObj != undefined) {
            ...
            swal.fire(
                'Quản lý việc làm gấp',
                'không thể cập nhật quản lý việc làm gấp, hãy kiểm tra lại thông tin.',
                'error'
            );
        }
    ...
    $("#topJobExtraModalTitle").text(id > 0 ? "Cập nhật quản lý việc làm gấp" : "Thêm mới quản lý việc làm gấp");
...
async function getItemById(id) {
    await $.ajax({
        url: systemConfig.defaultAPIURL + "api/top-job-urgent/detail/" + id,
        type: "GET",
...
function loadData() {
    $.ajax({
        url: systemConfig.defaultAPIURL + "api/top-job-urgent/list",
        type: "GET",
    ...
    $.ajax({
        ...
        error: function (e) {
            if (e.status === 401) {
                ...
                Swal.fire(
                    'Quản lý công việc',
                    'Không thể hiển thị công việc, hãy kiểm tra lại thông tin.',
                    'error'
                );
            }
        }
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa quản lý việc làm gấp',
        html: 'Bạn có chắc chắn muốn xóa quản lý việc làm gấp <b>' + item.jobName + '</b>?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $("#loading").addClass("show");
            $.ajax({
                url: systemConfig.defaultAPIURL + "api/top-job-urgent/delete?id=" + id,
                type: "DELETE",
                success: function (response) {
                    $("#loading").removeClass("show");
                    if (response.isSucceeded) {
                        Swal.fire(
                            'Xóa quản lý việc làm gấp',
                            'Quản lý việc làm gấp <b>' + updatingObj.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        reGenTable();
                    }
                        ...
                        if (response.status == 400) {
                            if (response.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Quản lý việc làm gấp <p class="swal__admin__subtitle"> Xóa không thành công </p>',
                                    contentError,
                                    'warning'
                                );
                            } else {
                ...
                error: function (e) {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Quản lý việc làm gấp',
                            'Quản lý việc làm gấp không thành công, <br> vui lòng thử lại sau!',
                            'error'
                        );
                    }
        ...
        swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                if (obj) {
                    try {
                        let result;
                        result = await httpService.postAsync("api/top-job-urgent/change-order-slide", obj);
...
function submit() {
    ...
    swal.fire({
        title: actionName + " quản lý việc làm gấp",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " việc làm gấp  <b>" + $("#selectJob option:selected").text() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#443',
        ...
        if (result.isConfirmed) {
            ...
            if (updatingId > 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/top-job-urgent/update",
                    type: "PUT",
                    contentType: "application/json",
                    ...
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Cập nhật việc làm gấp',
                                'Việc làm gấp <b>' + $("#selectJob option:selected").text() + '</b> đã được cập nhật thành công.',
                                'success'
                            ).then((result) => {
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Quản lý việc làm gấp <p class="swal__admin__subtitle"> Cập nhật không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
            ...
            if (updatingId == 0) {
                $.ajax({
                    url: systemConfig.defaultAPIURL + "api/top-job-urgent/add",
                    type: "POST",
                    contentType: "application/json",
                    data: JSON.stringify(obj),
                    success: function (response) {
                        ...
                        if (response.isSucceeded) {
                            Swal.fire(
                                'Thêm mới quản lý việc làm gấp',
                                'Quản lý việc làm gấp <b>' + $("#jobName").val() + '</b> đã được thêm mới thành công.',
                                'success'
                            ).then(function () {
                                $("#topJobExtraModal").modal("hide");
                            ...
                            if (response.status == 400) {
                                if (response.errors != null) {
                                    ...
                                    contentError += "</ul>";
                                    Swal.fire(
                                        'Quản lý việc làm gấp <p class="swal__admin__subtitle"> Thêm mới không thành công </p>',
                                        contentError,
                                        'warning'
                                    );
                                } else {
                    ...
                    error: function (e) {
                        ...
                        if (e.status === 401) {
                            ...
                            Swal.fire(
                                'Quản lý việc làm gấp',
                                'Không thể thêm mới quản lý việc làm gấp, hãy kiểm tra lại thông tin.',
                                'error'
                            );
                        }
                    }

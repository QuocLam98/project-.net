...
$(document).ready(async function () {
    ...
    $("#form-submit-candidateCVPDFType").on("submit", function (e) {
        e.preventDefault();
        validate();
    })
...
$("#candidateCVPDFTypeName").on("keypress", function (e) {
    if (e.keyCode == 13) {
...
async function editItem(id) {
    if (id > 0) {
        try {
            let result = await httpService.getAsync(`api/candidate-CV-PDF-type/detail/${id}`);
    ...
    $("#candidateCVPDFTypeName").val(id > 0 ? updatingObj.name : "").trigger("change");
    $("#candidateCVPDFTypeDescription").val(id > 0 ? updatingObj.description : "").trigger("change");
    ...
    $("#candidateCVPDFTypeModalTitle").text(id > 0 ? "Cập nhật loại hồ sơ ứng viên PDF " : "Thêm mới loại hồ sơ ứng viên PDF ");
    $("#loading").removeClass("show");
    $("#candidateCVPDFTypeModal").modal("show");
}


async function loadData() {
    try {
        let res = await httpService.getAsync("api/candidate-CV-PDF-type/list");
        if (res.isSucceeded) {
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa loại hồ sơ ứng viên PDF ',
        html: 'Bạn có chắc chắn muốn xóa loại hồ sơ ứng viên PDF  <b>' + item.name + '</b>',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            try {
                let result = await httpService.deleteAsync(`api/candidate-CV-PDF-type/delete/${id}`)
                if (result.isSucceeded) {
                    if (result.status == 200) {
                        Swal.fire(
                            'Loại hồ sơ ứng viên PDF',
                            'Loại hồ sơ ứng viên PDF  <b>' + item.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        refreshTable();
                    }
                    ...
                    Swal.fire(
                        'Loại hồ sơ ứng viên PDF',
                        'Xóa loại hồ sơ ứng viên PDF không thành công, <b> vui lòng thử lại sau.',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
                ...
                if (e.status === 401) {
                    ...
                    Swal.fire(
                        'Loại hồ sơ ứng viên PDF',
                        'Xóa loại hồ sơ ứng viên PDF không thành công, <br> vui lòng thử lại sau!',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
...
function submit() {
    ...
    updatingObj.name = $("#candidateCVPDFTypeName").val().trim();
    updatingObj.description = $("#candidateCVPDFTypeDescription").val();

    let actionName = updatingObj.id != undefined ? "Cập nhật" : "Thêm mới";

    Swal.fire({
        title: actionName + " loại hồ sơ ứng viên PDF ",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " loại hồ sơ ứng viên PDF <b>" + $("#candidateCVPDFTypeName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingObj) {
                try {
                    let result;
                    if (updatingObj.id > 0) {
                        result = await httpService.putAsync("api/candidate-CV-PDF-type/update", updatingObj);
                    } else {
                        result = await httpService.postAsync("api/candidate-CV-PDF-type/add", updatingObj);
                    }

                    if (result.isSucceeded) {
                        Swal.fire(
                            'Loại hồ sơ ứng viên PDF',
                            'Loại hồ sơ ứng viên PDF  <b>' + updatingObj.name + `</b> đã được ${actionName.toLowerCase()} thành công.`,
                            'success'
                        ).then((result) => {
                            $("#candidateCVPDFTypeModal").modal("hide");
                            refreshTable();
                        });
                    } else {
                        if (result.status == 400) {
                            ...
                            if (result.errors != null) {
                                ...
                                contentError += "</ul>";
                                Swal.fire(
                                    'Loại hồ sơ ứng viên PDF ' + swalSubTitle,
                                    contentError,
                                    'warning'
                                );
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Loại hồ sơ ứng viên PDF',
                            'Bạn không có loại hồ sơ ứng viên PDF  sử dụng tính năng này.',
                            'error'
                        );
                    }
                    else {
                        swal.fire(
                            'Loại hồ sơ ứng viên PDF ',
                            `${actionName} loại hồ sơ ứng viên PDF không thành công, <br> vui lòng thử lại sau!`,
                            'error'
                        );
                    }
                    $("#loading").removeClass("show");
...
function validate(obj) {
    var errorList = [];
    //if ($("#candidateCVPDFTypeName").val().length === 0) {
    //    errorList.push("Tên không được để trống.");
    //} else if ($("#candidateCVPDFTypeName").val().length > 255) {
    //    errorList.push("Tên không được dài quá 255 ký tự.");
    //}

    //if ($("#candidateCVPDFTypeValue").val().length === 0) {
    //    errorList.push("Giá trị không được để trống.");
    //} else if ($("#candidateCVPDFTypeValue").val().length > 255) {
    //    errorList.push("Giá trị không được dài quá 255 ký tự.");
    //}

    //if ($("#candidateCVPDFTypeDescription").val().length > 500) {

...
$(document).ready(async function () {
    ...
    $("#form-submit-jobType").on("submit", function (e) {
        e.preventDefault();
        validate();
    })
...
$("#jobTypeName").on("keypress", function (e) {
    if (e.keyCode == 13) {
...
async function editItem(id) {
    if (id > 0) {
        try {
            let result = await httpService.getAsync(`api/job-type/detail/${id}`);
    ...
    $("#jobTypeName").val(id > 0 ? updatingObj.name : "").trigger("change");
    $("#jobTypeDescription").val(id > 0 ? updatingObj.description : "").trigger("change");
    ...
    $("#jobTypeModalTitle").text(id > 0 ? "Cập nhật loại công việc" : "Thêm mới loại công việc");
...
async function deleteItem(id) {
    ...
    swal.fire({
        ...
        html: 'Bạn có chắc chắn muốn xóa loại công việc <b>' + item.name + '</b>',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            try {
                let result = await httpService.deleteAsync(`api/job-type/delete/${id}`)
                if (result.isSucceeded) {
                    if (result.status == 200) {
                        Swal.fire(
                            'Loại công việc',
                            'Loại công việc <b>' + item.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        refreshTable();
                    }
                    ...
                    Swal.fire(
                        'Loại công việc',
                        'Xóa loại công việc không thành công, <b> vui lòng thử lại sau.',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
                ...
                if (e.status === 401) {
                    ...
                    Swal.fire(
                        'Loại công việc',
                        'Xóa loại công việc không thành công, <br> vui lòng thử lại sau!',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
...
function submit() {
    ...
    updatingObj.name = $("#jobTypeName").val().trim();
    updatingObj.description = $("#jobTypeDescription").val();

    let actionName = updatingObj.id != undefined ? "Cập nhật" : "Thêm mới";

    Swal.fire({
        title: actionName + "loại công việc",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " loại công việc <b>" + $("#jobTypeName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingObj) {
                try {
                    let result;
                    if (updatingObj.id > 0) {
                        result = await httpService.putAsync("api/job-type/update", updatingObj);
                    } else {
                        result = await httpService.postAsync("api/job-type/add", updatingObj);
                    }

                    if (result.isSucceeded) {
                        Swal.fire(
                            'Loại công việc',
                            'Loại công việc <b>' + updatingObj.name + `</b> đã được ${actionName.toLowerCase()} thành công.`,
                            'success'
                        ).then((result) => {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Loại công việc',
                            'Bạn không có loại công việc sử dụng tính năng này.',
                            'error'
                        );
                    }
                    else {
                        swal.fire(
                            'Loại công việc',
                            `${actionName} loại công việc không thành công, <br> vui lòng thử lại sau!`,
                            'error'
                        );
                    }
                    $("#loading").removeClass("show");

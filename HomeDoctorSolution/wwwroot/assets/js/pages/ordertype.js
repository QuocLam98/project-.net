...
$(document).ready(async function () {
    ...
    $("#form-submit-orderType").on("submit", function (e) {
        e.preventDefault();
        validate();
    })
...
$("#orderTypeName").on("keypress", function (e) {
    if (e.keyCode == 13) {
...
async function editItem(id) {
    if (id > 0) {
        try {
            let result = await httpService.getAsync(`api/order-type/detail/${id}`);
    ...
    $("#orderTypeName").val(id > 0 ? updatingObj.name : "").trigger("change");
    $("#orderTypeDescription").val(id > 0 ? updatingObj.description : "").trigger("change");
    ...
    $("#orderTypeModalTitle").text(id > 0 ? "Cập nhật loại đơn hàng" : "Thêm mới loại đơn hàng");
...
async function loadData() {
    try {
        let res = await httpService.getAsync("api/order-type/list");
        if (res.isSucceeded) {
...
async function deleteItem(id) {
    ...
    swal.fire({
        title: 'Xóa loại đơn hàng',
        html: 'Bạn có chắc chắn muốn xóa loại đơn hàng <b>' + item.name + '</b>',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            try {
                let result = await httpService.deleteAsync(`api/order-type/delete/${id}`)
                if (result.isSucceeded) {
                    if (result.status == 200) {
                        Swal.fire(
                            'Loại đơn hàng',
                            'Loại đơn hàng <b>' + item.name + ' </b> đã được xóa thành công.',
                            'success'
                        );
                        refreshTable();
                    }
                    ...
                    Swal.fire(
                        'Loại đơn hàng',
                        'Xóa loại đơn hàng không thành công, <b> vui lòng thử lại sau.',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
                ...
                if (e.status === 401) {
                    ...
                    Swal.fire(
                        'Loại đơn hàng',
                        'Xóa loại đơn hàng không thành công, <br> vui lòng thử lại sau!',
                        'error'
                    );
                }
                $("#loading").removeClass("show");
...
function submit() {
    ...
    updatingObj.name = $("#orderTypeName").val().trim();
    updatingObj.description = $("#orderTypeDescription").val();

    let actionName = updatingObj.id != undefined ? "Cập nhật" : "Thêm mới";

    Swal.fire({
        title: actionName + "loại đơn hàng",
        html: "Bạn có chắc chắn muốn " + actionName.toLowerCase() + " loại đơn hàng <b>" + $("#orderTypeName").val() + '</b>?',
        icon: 'info',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            if (updatingObj) {
                try {
                    let result;
                    if (updatingObj.id > 0) {
                        result = await httpService.putAsync("api/order-type/update", updatingObj);
                    } else {
                        result = await httpService.postAsync("api/order-type/add", updatingObj);
                    }

                    if (result.isSucceeded) {
                        Swal.fire(
                            'Loại đơn hàng',
                            'Loại đơn hàng <b>' + updatingObj.name + `</b> đã được ${actionName.toLowerCase()} thành công.`,
                            'success'
                        ).then((result) => {
                    ...
                    if (e.status === 401) {
                        ...
                        Swal.fire(
                            'Loại đơn hàng',
                            'Bạn không có loại đơn hàng sử dụng tính năng này.',
                            'error'
                        );
                    }
                    else {
                        swal.fire(
                            'Loại đơn hàng',
                            `${actionName} loại đơn hàng không thành công, <br> vui lòng thử lại sau!`,
                            'error'
                        );
                    }
                    $("#loading").removeClass("show");

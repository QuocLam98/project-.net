...
namespace HomeDoctorSolution.Controllers
{
    ...
    public class DistrictController : BaseController
    {
        ...
        public DistrictController(IDistrictService _service, ICacheHelper cacheHelper) : base(cacheHelper)
        {
            service = _service;
        }
        [HttpGet]
        ...
        [HttpPost]
        [Route("api/Add")]
        public async Task<IActionResult> Add([FromBody] District model)
        {
        ...
        [HttpPost]
        [Route("api/Delete")]
        public async Task<IActionResult> Delete([FromBody] District model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        public async Task<IActionResult> DeletePermanently([FromBody] District model)
        {
            var result = 0;
            if (!(model.Id > 0))
            {
        ...
        public async Task<IActionResult> ListServerSide([FromBody] DistrictDTParameters parameters)
        {
            try
            {
        ...
        [HttpGet]
        [Route("api/ListByProvinceId/{id}")]
        public async Task<IActionResult> ListByProvinceId(int id)
        {
            try
            {
                var dataList = await service.ListByProvinceId(id);
                var homedoctorsolutionResponse = HomeDoctorResponse.SUCCESS(dataList.Cast<object>().ToList());
                return Ok(homedoctorsolutionResponse);
            }
            catch (Exception e)

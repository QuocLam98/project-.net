...
namespace HomeDoctorSolution.Controllers
{
    ...
    public class WardController : BaseController
    {
        ...
        public WardController(IWardService _service, ICacheHelper cacheHelper) : base(cacheHelper)
        {
            service = _service;
        }
        [HttpGet]
        ...
        [HttpPost]
        [Route("api/Add")]
        public async Task<IActionResult> Add([FromBody] Ward model)
        {
        ...
        [HttpPost]
        [Route("api/Update")]
        public async Task<IActionResult> Update([FromBody] Ward model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        [HttpPost]
        [Route("api/Delete")]
        public async Task<IActionResult> Delete([FromBody] Ward model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        public async Task<IActionResult> DeletePermanently([FromBody] Ward model)
        {
            var result = 0;
            if (!(model.Id > 0))
            {
        ...
        public async Task<IActionResult> ListServerSide([FromBody] WardDTParameters parameters)
        {
            try
            {
        ...
        [HttpGet]
        [Route("api/ListByDistrictId/{id}")]
        public async Task<IActionResult> ListByDistrictId(int id)
        {
            try
            {
                var dataList = await service.ListByDistrictId(id);
                var homedoctorsolutionResponse = HomeDoctorResponse.SUCCESS(dataList.Cast<object>().ToList());
                return Ok(homedoctorsolutionResponse);
            }
            catch (Exception)

...
namespace HomeDoctorSolution.Controllers
    {
        ...
        public class NotificationController : BaseController
        {
            ...
            public NotificationController(INotificationService _service, ICacheHelper cacheHelper )  : base(cacheHelper)
            {
                service = _service;
            }
            [HttpGet]
            ...
            public async Task<IActionResult> TestPushNotification([FromBody]PushNotiRequest model)
            {
                try
                {
                    await service.PushNotificationFCM(model.accountId, model.title, model.message, model.id, model.key);
                    return Ok();
                }
                catch (Exception)
                {
        ...
        [HttpGet]
            ...
            public async Task<IActionResult> SetDeviceFireBase(FireBaseDTO model)
            {
                try
                {
                    var accountId = this.GetLoggedInUserId();
                    var happysResponse = await service.SetDeviceFireBase(model, accountId);

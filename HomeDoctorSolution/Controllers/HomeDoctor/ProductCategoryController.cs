    ...
    namespace HomeDoctorSolution.Controllers
    {
        ...
        public class ProductCategoryController : BaseController
        {
            ...
            public ProductCategoryController(IProductCategoryService _service, ICacheHelper cacheHelper )  : base(cacheHelper)
            {
                service = _service;
            }
            [HttpGet]
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]ProductCategory model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]ProductCategory model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]ProductCategory model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]ProductCategory model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] ProductCategoryDTParameters parameters)
            {
                try
                {

    ...
    namespace HomeDoctorSolution.Controllers
    {
        ...
        public class CartController : BaseController
        {
            ...
            public CartController(ICartService _service, ICacheHelper cacheHelper )  : base(cacheHelper)
            {
                service = _service;
            }
            [HttpGet]
            ...
            public async Task<IActionResult> Search(string keyword)
            {
                try
                {
                    var cart = await service.Search(keyword);
                    if (cart == null || cart.Count == 0)
                    {
                        return NotFound();
                    }

                    var cartProduct = await service.GetProductsInCart(cart[0].Id);
                    if (cartProduct == null)
                    {
                        return NotFound();
                    }

                    var homedoctorsolutionResponse = HomeDoctorResponse.SUCCESS(cartProduct);
                    return Ok(homedoctorsolutionResponse);
                }
                catch (Exception)
                {
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]Cart model)
            {
            ...
            [HttpPost]
            [Route("api/DeletePermanently/{id}")]
            public async Task<IActionResult> DeletePermanently(int? id)
            {
                ...
                try
                {
                    ...
                    result = await service.DeletePermanently(id);
                    if (result == 0)
                    {
                        return NotFound();
                    }
            ...
            public async Task<IActionResult> ListServerSide([FromBody] CartDTParameters parameters)
            {
                try
                {
            ...
            public async Task<IActionResult> AddCartProduct()
            {
                if (!ModelState.IsValid)
                {
                    return BadRequest();
                ...
                try
                {
                    ...
                    var result = await service.CartInfo(accountId);
                    if (result == null)
                    {
                        return NotFound();
                    }

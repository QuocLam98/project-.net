    ...
    namespace HomeDoctorSolution.Controllers
    {
        [Route("[controller]")]
        [ApiController]
        public class SurveySectionQuestionController : BaseController
        {
            ISurveySectionQuestionService service;
            public SurveySectionQuestionController(ISurveySectionQuestionService _service, ICacheHelper cacheHelper )  : base(cacheHelper)
            {
                service = _service;
            }
            [HttpGet]
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]SurveySectionQuestion model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]SurveySectionQuestion model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]SurveySectionQuestion model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]SurveySectionQuestion model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] SurveySectionQuestionDTParameters parameters)
            {
                try
                {

    ...
    namespace HomeDoctorSolution.Controllers
    {
        [Route("[controller]")]
        [ApiController]
        public class SurveyAccountShareLinkController : BaseController
        {
            ISurveyAccountShareLinkService service;
            public SurveyAccountShareLinkController(ISurveyAccountShareLinkService _service, ICacheHelper cacheHelper )  : base(cacheHelper)
            {
                service = _service;
            }
            [HttpGet]
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]SurveyAccountShareLink model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]SurveyAccountShareLink model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]SurveyAccountShareLink model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]SurveyAccountShareLink model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            [HttpGet]
            [Route("api/Count")]
            public int CountSurveyAccountShareLink()
            {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] SurveyAccountShareLinkDTParameters parameters)
            {
                try
                {

...
namespace HomeDoctorSolution.Controllers
{
    ...
    public class ProvinceController : BaseController
    {
        ...
        public ProvinceController(IProvinceService _service, ICacheHelper cacheHelper) : base(cacheHelper)
        {
            service = _service;
        }
        [HttpGet]
        ...
        [HttpPost]
        [Route("api/Add")]
        public async Task<IActionResult> Add([FromBody] Province model)
        {
        ...
        [HttpPost]
        [Route("api/Delete")]
        public async Task<IActionResult> Delete([FromBody] Province model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        public async Task<IActionResult> DeletePermanently([FromBody] Province model)
        {
            var result = 0;
            if (!(model.Id > 0))
            {
        ...
        public async Task<IActionResult> ListServerSide([FromBody] ProvinceDTParameters parameters)
        {
            try
            {
        ...
        [HttpGet]
        [Route("api/listProvinces")]
        public async Task<Provinces[]> GetListProvinces()
        {
            var client = new HttpClient();
            var res = await client.GetAsync("https://provinces.open-api.vn/api/p/");
            res.EnsureSuccessStatusCode();
            var json = await res.Content.ReadAsStringAsync();
            var obj = JsonConvert.DeserializeObject<Provinces[]>(json);
            return obj;
        }

        [HttpGet]
        [Route("api/listDistricts")]
        public async Task<Districts[]> GetListDistricts()
        {
            var client = new HttpClient();
            var res = await client.GetAsync("https://provinces.open-api.vn/api/d/");
            res.EnsureSuccessStatusCode();
            var json = await res.Content.ReadAsStringAsync();
            var obj = JsonConvert.DeserializeObject<Districts[]>(json);
            return obj;
        }

        [HttpGet]
        [Route("api/listWards")]
        public async Task<Wards[]> GetListWards()
        {
            var client = new HttpClient();
            var res = await client.GetAsync("https://provinces.open-api.vn/api/w/");
            res.EnsureSuccessStatusCode();
            var json = await res.Content.ReadAsStringAsync();
            var obj = JsonConvert.DeserializeObject<Wards[]>(json);
            return obj;
        }
    }
}

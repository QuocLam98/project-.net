    ...
    namespace HomeDoctorSolution.Controllers
    {
        [Route("[controller]")]
        [ApiController]
        public class FavouritePostController : BaseController
        {
            IFavouritePostService service;
            public FavouritePostController(IFavouritePostService _service, ICacheHelper cacheHelper )  : base(cacheHelper)
            {
                service = _service;
            }
            [HttpGet]
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]FavouritePost model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]FavouritePost model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]FavouritePost model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]FavouritePost model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] FavouritePostDTParameters parameters)
            {
                try
                {

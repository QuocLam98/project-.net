    ...
    namespace HomeDoctorSolution.Controllers
    {
        [Route("[controller]")]
        [ApiController]
        public class ReadedPostController : BaseController
        {
            IReadedPostService service;
            public ReadedPostController(IReadedPostService _service, ICacheHelper cacheHelper )  : base(cacheHelper)
            {
                service = _service;
            }
            [HttpGet]
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]ReadedPost model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]ReadedPost model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]ReadedPost model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]ReadedPost model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] ReadedPostDTParameters parameters)
            {
                try
                {

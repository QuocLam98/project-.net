...
namespace HomeDoctorSolution.Controllers
{
    ...
    public class AnamnesisController : BaseController
    {
        ...
        public AnamnesisController(IAnamnesisService _service, ICacheHelper cacheHelper) : base(cacheHelper)
        {
            service = _service;
        }
        [HttpGet]
        ...
        [HttpPost]
        [Route("api/Add")]
        public async Task<IActionResult> Add([FromBody] Anamnesis model)
        {
        ...
        [HttpPost]
        [Route("api/Update")]
        public async Task<IActionResult> Update([FromBody] Anamnesis model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        [HttpPost]
        [Route("api/Delete")]
        public async Task<IActionResult> Delete([FromBody] Anamnesis model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        public async Task<IActionResult> DeletePermanently([FromBody] Anamnesis model)
        {
            var result = 0;
            if (!(model.Id > 0))
            {
        ...
        public async Task<IActionResult> ListServerSide([FromBody] AnamnesisDTParameters parameters)
        {
            try
            {
        ...
        [HttpPost]
        [Route("api/AddOrUpdateAnamnessis")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        public async Task<IActionResult> AddOrUpdateAnamnessis([FromBody] Anamnesis model)
        {
            try
            {
                var response = await service.AddOrUpdateAnamnessis(model);
                return Ok(response);
            }
            catch (Exception ex)
            {

    ...
    namespace HomeDoctorSolution.Controllers
    {
        ...
        {
            ILanguageConfigService service;
            public LanguageConfigController(ILanguageConfigService _service, ICacheHelper cacheHelper )  : base(cacheHelper)
            {
                service = _service;
            }
            [HttpGet]
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]LanguageConfig model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]LanguageConfig model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]LanguageConfig model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]LanguageConfig model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            [HttpGet]
            [Route("api/Count")]
            public int CountLanguageConfig()
            {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] LanguageConfigDTParameters parameters)
            {
                try
                {

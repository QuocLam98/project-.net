...
namespace HomeDoctorSolution.Controllers
{
    ...
    {
        IHealthFacilityService service;
        public HealthFacilityController(IHealthFacilityService _service, ICacheHelper cacheHelper) : base(cacheHelper)
        {
            service = _service;
        }
        [HttpGet]
        ...
        [HttpGet]
        [Route("api/ListPagingView")]
        public async Task<IActionResult> ListPagingView(int pageIndex, int pageSize, int provinceId, string keyword)
        {
            if (pageIndex < 0 || pageSize < 0) return BadRequest();
            try
            {
                var dataList = await service.ListPagingView(pageIndex, pageSize, provinceId, keyword);
        ...
        [HttpPost]
        [Route("api/Add")]
        public async Task<IActionResult> Add([FromBody] HealthFacility model)
        {
        ...
        [HttpPost]
        [Route("api/Update")]
        public async Task<IActionResult> Update([FromBody] HealthFacility model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        [HttpPost]
        [Route("api/Delete")]
        public async Task<IActionResult> Delete([FromBody] HealthFacility model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        public async Task<IActionResult> DeletePermanently([FromBody] HealthFacility model)
        {
            var result = 0;
            if (!(model.Id > 0))
            {
        ...
        [HttpGet]
        [Route("api/Count")]
        public int CountHealthFacility()
        {
        ...
        public async Task<IActionResult> ListServerSide([FromBody] HealthFacilityDTParameters parameters)
        {
            try
            {

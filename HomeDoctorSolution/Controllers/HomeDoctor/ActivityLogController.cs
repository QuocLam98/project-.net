    ...
    namespace HomeDoctorSolution.Controllers
    {
        ...
        public class ActivityLogController : BaseController
        {
            ...
            public ActivityLogController(IActivityLogService _service, ICacheHelper cacheHelper )  : base(cacheHelper)
            {
                service = _service;
            }
            [HttpGet]
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]ActivityLog model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]ActivityLog model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]ActivityLog model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]ActivityLog model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] ActivityLogDTParameters parameters)
            {
                try
                {

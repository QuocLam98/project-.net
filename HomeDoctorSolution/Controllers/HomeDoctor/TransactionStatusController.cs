    ...
    namespace HomeDoctorSolution.Controllers
    {
        ...
        public class TransactionStatusController : BaseController
        {
            ...
            public TransactionStatusController(ITransactionStatusService _service, ICacheHelper cacheHelper )  : base(cacheHelper)
            {
                service = _service;
            }
            [HttpGet]
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]TransactionStatus model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]TransactionStatus model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]TransactionStatus model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]TransactionStatus model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            [HttpGet]
            [Route("api/Count")]
            public int CountTransactionStatus()
            {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] TransactionStatusDTParameters parameters)
            {
                try
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class ProductCategoryRepository: IProductCategoryRepository
                {
                    ...
                    public ProductCategoryRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<ProductCategory>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.ProductCategories
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.ProductCategories.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.ProductCategories.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.ProductCategories.FirstOrDefaultAsync(x => x.Id == objId);
            ...
            public async Task <DTResult<ProductCategory>> ListServerSide(ProductCategoryDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.ProductCategories 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new ProductCategory()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<ProductCategory>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<ProductCategory>()
                {

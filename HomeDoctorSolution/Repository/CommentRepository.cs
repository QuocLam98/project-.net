    ...
    namespace HomeDoctorSolution.Repository
        {
            public class CommentRepository: ICommentRepository
                {
                    ...
                    public CommentRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.Comments.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
...
db.Entry(obj).Property(x => x.CommentStatusId).IsModified = true;
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task <DTResult<CommentViewModel>> ListServerSide(CommentDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.Comments 

                                    join p in db.Posts on row.PostId equals p.Id 
join a in db.Accounts on row.AccountId equals a.Id 
join cs in db.CommentStatuses on row.CommentStatusId equals cs.Id 
...
case "createdTime":
                ...
                break;
                                ...
                                {
                                    query = query.Where(c => parameters.PostIds.Contains(c.row.PostId));
                                }
                                ...
                                if (parameters.CommentStatusIds.Count > 0)
                                {
                                    query = query.Where(c => parameters.CommentStatusIds.Contains(c.row.CommentStatus.Id));
                                }
                                

                //3.Query second
                var query2 = query.Select(c => new CommentViewModel()
                {
                    Id = c.row.Id,
PostId = c.p.Id,
PostName = c.p.Name,
...
Active = c.row.Active,
CommentStatusId = c.cs.Id,
CommentStatusName = c.cs.Name,
Name = c.row.Name,
Email = c.row.Email,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<CommentViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<CommentViewModel>()
                {

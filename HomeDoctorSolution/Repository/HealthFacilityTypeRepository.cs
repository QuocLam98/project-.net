    ...
    namespace HomeDoctorSolution.Repository
        {
            public class HealthFacilityTypeRepository: IHealthFacilityTypeRepository
                {
                    HomeDoctorContext db;
                    public HealthFacilityTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<HealthFacilityType>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.HealthFacilityTypes
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.HealthFacilityTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <HealthFacilityType> Add(HealthFacilityType obj)
            {
                if (db != null) {
                    await db.HealthFacilityTypes.AddAsync(obj);
            ...
            public async Task Update(HealthFacilityType obj)
            {
                if (db != null) {
                    //Update that object
                    db.HealthFacilityTypes.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.HealthFacilityTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.HealthFacilityTypes.Remove(obj);
            ...
            public async Task <DTResult<HealthFacilityType>> ListServerSide(HealthFacilityTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.HealthFacilityTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new HealthFacilityType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<HealthFacilityType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<HealthFacilityType>()
                {

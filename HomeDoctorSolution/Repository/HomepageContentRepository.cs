    ...
    namespace HomeDoctorSolution.Repository
        {
            public class HomepageContentRepository: IHomepageContentRepository
                {
                    ...
                    public HomepageContentRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<HomepageContent>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.HomepageContents
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.HomepageContents.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <HomepageContent> Add(HomepageContent obj)
            {
                if (db != null) {
                    await db.HomepageContents.AddAsync(obj);
            ...
            public async Task Update(HomepageContent obj)
            {
                if (db != null) {
                    //Update that object
                    db.HomepageContents.Attach(obj);
                    db.Entry(obj).Property(x => x.HomepageContentTypeId).IsModified = true;
...
db.Entry(obj).Property(x => x.Text).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.HomepageContents.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.HomepageContents.Remove(obj);
            ...
            public async Task <DTResult<HomepageContentViewModel>> ListServerSide(HomepageContentDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.HomepageContents 

                                    join hct in db.HomepageContentTypes on row.HomepageContentTypeId equals hct.Id 
...
case "createdTime":
                ...
                break;
                                ...
                                if (parameters.HomepageContentTypeIds.Count > 0)
                                {
                                    query = query.Where(c => parameters.HomepageContentTypeIds.Contains(c.row.HomepageContentType.Id));
                                }
                                

                //3.Query second
                var query2 = query.Select(c => new HomepageContentViewModel()
                {
                    Id = c.row.Id,
HomepageContentTypeId = c.hct.Id,
HomepageContentTypeName = c.hct.Name,
Active = c.row.Active,
Name = c.row.Name,
Url = c.row.Url,
Photo = c.row.Photo,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<HomepageContentViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<HomepageContentViewModel>()
                {

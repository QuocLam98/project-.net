    ...
    namespace HomeDoctorSolution.Repository
        {
            public class HealthFacilityStatusRepository: IHealthFacilityStatusRepository
                {
                    HomeDoctorContext db;
                    public HealthFacilityStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<HealthFacilityStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.HealthFacilityStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.HealthFacilityStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <HealthFacilityStatus> Add(HealthFacilityStatus obj)
            {
                if (db != null) {
                    await db.HealthFacilityStatuses.AddAsync(obj);
            ...
            public async Task Update(HealthFacilityStatus obj)
            {
                if (db != null) {
                    //Update that object
                    db.HealthFacilityStatuses.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.HealthFacilityStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.HealthFacilityStatuses.Remove(obj);
            ...
            public async Task <DTResult<HealthFacilityStatus>> ListServerSide(HealthFacilityStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.HealthFacilityStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new HealthFacilityStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<HealthFacilityStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<HealthFacilityStatus>()
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class OrderStatusShipRepository: IOrderStatusShipRepository
                {
                    HomeDoctorContext db;
                    public OrderStatusShipRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<OrderStatusShip>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.OrderStatusShips
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.OrderStatusShips.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <OrderStatusShip> Add(OrderStatusShip obj)
            {
                if (db != null) {
                    await db.OrderStatusShips.AddAsync(obj);
            ...
            public async Task Update(OrderStatusShip obj)
            {
                if (db != null) {
                    //Update that object
                    db.OrderStatusShips.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.OrderStatusShips.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.OrderStatusShips.Remove(obj);
            ...
            public async Task <DTResult<OrderStatusShip>> ListServerSide(OrderStatusShipDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.OrderStatusShips 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new OrderStatusShip()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<OrderStatusShip>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<OrderStatusShip>()
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class RoomRepository: IRoomRepository
                {
                    ...
                    public RoomRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.Rooms.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <Room> Add(Room obj)
            {
                if (db != null) {
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.Rooms.FirstOrDefaultAsync(x => x.Id == objId);
            ...
            public async Task <DTResult<Room>> ListServerSide(RoomDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.Rooms 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new Room()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<Room>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<Room>()
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class NotificationStatusRepository: INotificationStatusRepository
                {
                    ...
                    public NotificationStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.NotificationStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task <DTResult<NotificationStatus>> ListServerSide(NotificationStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.NotificationStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new NotificationStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<NotificationStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<NotificationStatus>()
                {

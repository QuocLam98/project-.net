    ...
    namespace HomeDoctorSolution.Repository
        {
            public class ShipAddressRepository: IShipAddressRepository
                {
                    ...
                    public ShipAddressRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.ShipAddresses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <ShipAddress> Add(ShipAddress obj)
            {
                if (db != null) {
                    await db.ShipAddresses.AddAsync(obj);
            ...
            public async Task Update(ShipAddress obj)
            {
                if (db != null) {
                    //Update that object
                    db.ShipAddresses.Attach(obj);
                    db.Entry(obj).Property(x => x.Active).IsModified = true;
db.Entry(obj).Property(x => x.Name).IsModified = true;
db.Entry(obj).Property(x => x.Description).IsModified = true;
db.Entry(obj).Property(x => x.WardId).IsModified = true;
...
db.Entry(obj).Property(x => x.RecipientName).IsModified = true;
db.Entry(obj).Property(x => x.RecipientPhoneNumber).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.ShipAddresses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.ShipAddresses.Remove(obj);
            ...
            public async Task <DTResult<ShipAddressViewModel>> ListServerSide(ShipAddressDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.ShipAddresses 

                                    join w in db.Wards on row.WardId equals w.Id 
...
EF.Functions.Collate(c.row.AddressDetail.ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General)) ||
EF.Functions.Collate(c.row.RecipientName.ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General)) ||
EF.Functions.Collate(c.row.RecipientPhoneNumber.ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General)) ||
...
case "active":
 ...
 case "addressDetail":
                ...
                break;
 case "recipientName":
                query = query.Where(c => (c.row.RecipientName ?? "").Contains(fillter));
                break;
 case "recipientPhoneNumber":
                query = query.Where(c => (c.row.RecipientPhoneNumber ?? "").Contains(fillter));
                break;
case "createdTime":
                if (fillter.Contains(" - "))
                {
                ...
                break;
                                ...
                                if (parameters.WardIds.Count > 0)
                                {
                                    query = query.Where(c => parameters.WardIds.Contains(c.row.Ward.Id));
                                }
                ...
                //3.Query second
                var query2 = query.Select(c => new ShipAddressViewModel()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
RecipientName = c.row.RecipientName,
RecipientPhoneNumber = c.row.RecipientPhoneNumber,
CreatedTime = c.row.CreatedTime,

                });
                //4. Sort
                query2 = query2.OrderByDynamic<ShipAddressViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<ShipAddressViewModel>()
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class LanguageConfigRepository: ILanguageConfigRepository
                {
                    ...
                    public LanguageConfigRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<LanguageConfig>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.LanguageConfigs
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.LanguageConfigs.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.LanguageConfigs.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.LanguageConfigs.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.LanguageConfigs.Remove(obj);
            ...
            public async Task <DTResult<LanguageConfig>> ListServerSide(LanguageConfigDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.LanguageConfigs 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new LanguageConfig()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<LanguageConfig>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<LanguageConfig>()
                {

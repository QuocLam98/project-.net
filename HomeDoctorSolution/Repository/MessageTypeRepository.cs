    ...
    namespace HomeDoctorSolution.Repository
        {
            public class MessageTypeRepository: IMessageTypeRepository
                {
                    ...
                    public MessageTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.MessageTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <MessageType> Add(MessageType obj)
            {
                if (db != null) {
                    await db.MessageTypes.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.MessageTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.MessageTypes.Remove(obj);
            ...
            public async Task <DTResult<MessageType>> ListServerSide(MessageTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.MessageTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new MessageType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<MessageType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<MessageType>()
                {

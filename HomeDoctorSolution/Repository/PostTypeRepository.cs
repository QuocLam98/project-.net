    ...
    namespace HomeDoctorSolution.Repository
        {
            public class PostTypeRepository: IPostTypeRepository
                {
                    ...
                    public PostTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.PostTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.PostTypes.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.PostTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.PostTypes.Remove(obj);
            ...
            public async Task <DTResult<PostType>> ListServerSide(PostTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.PostTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new PostType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<PostType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<PostType>()
                {

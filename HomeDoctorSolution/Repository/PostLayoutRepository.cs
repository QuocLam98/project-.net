    ...
    namespace HomeDoctorSolution.Repository
        {
            public class PostLayoutRepository: IPostLayoutRepository
                {
                    ...
                    public PostLayoutRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.PostLayouts.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.PostLayouts.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.PostLayouts.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.PostLayouts.Remove(obj);
            ...
            public async Task <DTResult<PostLayout>> ListServerSide(PostLayoutDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.PostLayouts 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new PostLayout()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<PostLayout>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<PostLayout>()
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class MenuTypeRepository: IMenuTypeRepository
                {
                    ...
                    public MenuTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.MenuTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <MenuType> Add(MenuType obj)
            {
                if (db != null) {
                    await db.MenuTypes.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.MenuTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.MenuTypes.Remove(obj);
            ...
            public async Task <DTResult<MenuType>> ListServerSide(MenuTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.MenuTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new MenuType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<MenuType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<MenuType>()
                {

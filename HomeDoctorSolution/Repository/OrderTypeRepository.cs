    ...
    namespace HomeDoctorSolution.Repository
        {
            public class OrderTypeRepository: IOrderTypeRepository
                {
                    ...
                    public OrderTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.OrderTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <OrderType> Add(OrderType obj)
            {
                if (db != null) {
                    await db.OrderTypes.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.OrderTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.OrderTypes.Remove(obj);
            ...
            public async Task <DTResult<OrderType>> ListServerSide(OrderTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.OrderTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new OrderType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<OrderType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<OrderType>()
                {

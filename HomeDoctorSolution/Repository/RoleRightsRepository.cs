...
namespace HomeDoctorSolution.Repository
{
    public class RoleRightsRepository : IRoleRightsRepository
    {
        ...
        public RoleRightsRepository(HomeDoctorContext _db)
        {
            db = _db;
        }
        ...
        public async Task<RoleRight> Detail(int? id)
        {
            if (db != null)
            {
                return await db.RoleRights.FirstOrDefaultAsync(x => x.Id == id && x.Active == 1);
            }
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
            ...
            {
                query = query.Where(c => parameters.RightsIds.Contains(c.row.Right.Id));
            }
        ...
        public async Task<List<RoleRight>> ListByRoleId(int? id)
        {
            if (db != null)
            {
                return await (
                    from row in db.RoleRights
                    where (row.Active == 1 && row.RoleId == id)
                    select row)
                    .ToListAsync();
            }
            return null;

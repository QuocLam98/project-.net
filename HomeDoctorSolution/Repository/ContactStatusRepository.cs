    ...
    namespace HomeDoctorSolution.Repository
        {
            public class ContactStatusRepository: IContactStatusRepository
                {
                    ...
                    public ContactStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<ContactStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.ContactStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.ContactStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <ContactStatus> Add(ContactStatus obj)
            {
                if (db != null) {
                    await db.ContactStatuses.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.ContactStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.ContactStatuses.Remove(obj);
            ...
            public async Task <DTResult<ContactStatus>> ListServerSide(ContactStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.ContactStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new ContactStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<ContactStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<ContactStatus>()
                {

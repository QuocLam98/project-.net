    ...
    namespace HomeDoctorSolution.Repository
        {
            public class AuthorRepository: IAuthorRepository
                {
                    ...
                    public AuthorRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.Authors.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <Author> Add(Author obj)
            {
                if (db != null) {
                    await db.Authors.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.Authors.FirstOrDefaultAsync(x => x.Id == objId);
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new AuthorViewModel()
                {
                    Id = c.row.Id,
AccountId = c.a.Id,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<AuthorViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<AuthorViewModel>()
                {

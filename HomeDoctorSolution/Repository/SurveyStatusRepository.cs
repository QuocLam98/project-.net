    ...
    namespace HomeDoctorSolution.Repository
        {
            public class SurveyStatusRepository: ISurveyStatusRepository
                {
                    HomeDoctorContext db;
                    public SurveyStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.SurveyStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <SurveyStatus> Add(SurveyStatus obj)
            {
                if (db != null) {
                    await db.SurveyStatuses.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.SurveyStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.SurveyStatuses.Remove(obj);
            ...
            public async Task <DTResult<SurveyStatus>> ListServerSide(SurveyStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.SurveyStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new SurveyStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<SurveyStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<SurveyStatus>()
                {

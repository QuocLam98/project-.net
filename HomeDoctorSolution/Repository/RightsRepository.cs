...
namespace HomeDoctorSolution.Repository
{
    public class RightsRepository : IRightsRepository
    {
        ...
        public RightsRepository(HomeDoctorContext _db)
        {
            db = _db;
        }
        ...
        public async Task<Right> Detail(int? id)
        {
            if (db != null)
            {
                return await db.Rights.FirstOrDefaultAsync(x => x.Id == id && x.Active == 1);
            }
        ...
        public async Task<Right> Add(Right obj)
        {
            if (db != null)
            {
                await db.Rights.AddAsync(obj);
        ...
        public async Task<int> DeletePermanently(int? objId)
        {
            ...
            if (db != null)
            {
                ...
                var obj = await db.Rights.FirstOrDefaultAsync(x => x.Id == objId);

                if (obj != null)
                {
                    //Delete that obj
                    db.Rights.Remove(obj);
        ...
        public async Task<DTResult<Right>> ListServerSide(RightsDTParameters parameters)
        {
            ...
            //1. Join
            var query = from row in db.Rights
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
            ...
            //3.Query second
            var query2 = query.Select(c => new Right()
            {
                Id = c.row.Id,
                Active = c.row.Active,
                Name = c.row.Name,
            ...
            //4. Sort
            query2 = query2.OrderByDynamic<Right>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<Right>()
            {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class TransactionStatusRepository: ITransactionStatusRepository
                {
                    ...
                    public TransactionStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<TransactionStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.TransactionStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.TransactionStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <TransactionStatus> Add(TransactionStatus obj)
            {
                if (db != null) {
                    await db.TransactionStatuses.AddAsync(obj);
            ...
            public async Task Update(TransactionStatus obj)
            {
                if (db != null) {
                    //Update that object
                    db.TransactionStatuses.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.TransactionStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.TransactionStatuses.Remove(obj);
            ...
            public async Task <DTResult<TransactionStatus>> ListServerSide(TransactionStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.TransactionStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new TransactionStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<TransactionStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<TransactionStatus>()
                {

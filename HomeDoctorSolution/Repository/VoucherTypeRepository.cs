    ...
    namespace HomeDoctorSolution.Repository
        {
            public class VoucherTypeRepository: IVoucherTypeRepository
                {
                    ...
                    public VoucherTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.VoucherTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <VoucherType> Add(VoucherType obj)
            {
                if (db != null) {
                    await db.VoucherTypes.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.VoucherTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.VoucherTypes.Remove(obj);
            ...
            public async Task <DTResult<VoucherType>> ListServerSide(VoucherTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.VoucherTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new VoucherType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<VoucherType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<VoucherType>()
                {

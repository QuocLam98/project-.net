    ...
    namespace HomeDoctorSolution.Repository
        {
            public class PromotionRepository: IPromotionRepository
                {
                    ...
                    public PromotionRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.Promotions.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <Promotion> Add(Promotion obj)
            {
                if (db != null) {
                    await db.Promotions.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.Promotions.FirstOrDefaultAsync(x => x.Id == objId);
            ...
            public async Task <DTResult<Promotion>> ListServerSide(PromotionDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.Promotions 
...
EF.Functions.Collate(c.row.Discount.ToString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General)) ||
...
case "active":
 ...
 case "code":
                ...
                break;
case "discount":
                query = query.Where(c => c.row.Discount.ToString().Trim().Contains(fillter));
                break; 
 case "description":
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new Promotion()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Code = c.row.Code,
Discount = c.row.Discount,
Description = c.row.Description,
CreatedTime = c.row.CreatedTime,

                });
                //4. Sort
                query2 = query2.OrderByDynamic<Promotion>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<Promotion>()
                {

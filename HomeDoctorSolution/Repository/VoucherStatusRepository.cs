    ...
    namespace HomeDoctorSolution.Repository
        {
            public class VoucherStatusRepository: IVoucherStatusRepository
                {
                    ...
                    public VoucherStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<VoucherStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.VoucherStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.VoucherStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.VoucherStatuses.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.VoucherStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.VoucherStatuses.Remove(obj);
            ...
            public async Task <DTResult<VoucherStatus>> ListServerSide(VoucherStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.VoucherStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new VoucherStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<VoucherStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<VoucherStatus>()
                {

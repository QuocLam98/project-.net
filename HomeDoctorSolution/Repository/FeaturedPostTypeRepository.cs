    ...
    namespace HomeDoctorSolution.Repository
        {
            public class FeaturedPostTypeRepository: IFeaturedPostTypeRepository
                {
                    HomeDoctorContext db;
                    public FeaturedPostTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<FeaturedPostType>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.FeaturedPostTypes
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.FeaturedPostTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <FeaturedPostType> Add(FeaturedPostType obj)
            {
                if (db != null) {
                    await db.FeaturedPostTypes.AddAsync(obj);
            ...
            public async Task Update(FeaturedPostType obj)
            {
                if (db != null) {
                    //Update that object
                    db.FeaturedPostTypes.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.FeaturedPostTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.FeaturedPostTypes.Remove(obj);
            ...
            public async Task <DTResult<FeaturedPostType>> ListServerSide(FeaturedPostTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.FeaturedPostTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new FeaturedPostType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<FeaturedPostType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<FeaturedPostType>()
                {

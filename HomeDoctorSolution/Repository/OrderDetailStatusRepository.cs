    ...
    namespace HomeDoctorSolution.Repository
        {
            public class OrderDetailStatusRepository: IOrderDetailStatusRepository
                {
                    HomeDoctorContext db;
                    public OrderDetailStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<OrderDetailStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.OrderDetailStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.OrderDetailStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <OrderDetailStatus> Add(OrderDetailStatus obj)
            {
                if (db != null) {
                    await db.OrderDetailStatuses.AddAsync(obj);
            ...
            public async Task Update(OrderDetailStatus obj)
            {
                if (db != null) {
                    //Update that object
                    db.OrderDetailStatuses.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.OrderDetailStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.OrderDetailStatuses.Remove(obj);
            ...
            public async Task <DTResult<OrderDetailStatus>> ListServerSide(OrderDetailStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.OrderDetailStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new OrderDetailStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<OrderDetailStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<OrderDetailStatus>()
                {

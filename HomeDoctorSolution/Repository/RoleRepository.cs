...
namespace HomeDoctorSolution.Repository
{
    public class RoleRepository : IRoleRepository
    {
        ...
        public RoleRepository(HomeDoctorContext _db)
        {
            db = _db;
        }
        ...
        public async Task<Role> Detail(int? id)
        {
            if (db != null)
            {
                return await db.Roles.FirstOrDefaultAsync(row => row.Id == id && row.Active == 1);
            }
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
        ...
        public async Task<int> GetIdByCode(string codeName)
        {
            int result = 0;
            if (db != null)
            {
                var data = await (
                    from row in db.Roles
                    where row.Active == 1 && row.CodeName == codeName
                    select row
                ).ToListAsync();
                if (data.Count > 0)
                {

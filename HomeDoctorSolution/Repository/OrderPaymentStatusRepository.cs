    ...
    namespace HomeDoctorSolution.Repository
        {
            public class OrderPaymentStatusRepository: IOrderPaymentStatusRepository
                {
                    ...
                    public OrderPaymentStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<OrderPaymentStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.OrderPaymentStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.OrderPaymentStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.OrderPaymentStatuses.AddAsync(obj);
            ...
            public async Task Update(OrderPaymentStatus obj)
            {
                if (db != null) {
                    //Update that object
                    db.OrderPaymentStatuses.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.OrderPaymentStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.OrderPaymentStatuses.Remove(obj);
            ...
            public async Task <DTResult<OrderPaymentStatus>> ListServerSide(OrderPaymentStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.OrderPaymentStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new OrderPaymentStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<OrderPaymentStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<OrderPaymentStatus>()
                {

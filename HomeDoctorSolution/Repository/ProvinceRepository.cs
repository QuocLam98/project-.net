...
namespace HomeDoctorSolution.Repository
{
    public class ProvinceRepository : IProvinceRepository
    {
        ...
        public ProvinceRepository(HomeDoctorContext _db)
        {
            db = _db;
        }
        ...
        public async Task<Province> Detail(int? id)
        {
            if (db != null)
            {
                return await db.Provinces.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
            }
            return null;
        }
        ...
        public async Task<Province> Add(Province obj)
        {
            if (db != null)
            {
                await db.Provinces.AddAsync(obj);
        ...
        public async Task Update(Province obj)
        {
            ...
            {
                //Update that object
                db.Provinces.Attach(obj);
                //db.Entry(obj).Property(x => x.CountryId).IsModified = true;
        ...
        public async Task<int> DeletePermanently(int? objId)
        {
            ...
            if (db != null)
            {
                ...
                var obj = await db.Provinces.FirstOrDefaultAsync(x => x.Id == objId);
        ...
        public async Task<DTResult<ProvinceViewModel>> ListServerSide(ProvinceDTParameters parameters)
        {
            ...
            //1. Join
            var query = from row in db.Provinces

                        //join c in db.Countries on row.CountryId equals c.Id
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
            ...
            //if (parameters.CountryIds.Count > 0)
            //{
            //    query = query.Where(c => parameters.CountryIds.Contains(c.row.Country.Id));
            //}


            //3.Query second
            var query2 = query.Select(c => new ProvinceViewModel()
            {
                Id = c.row.Id,
                //CountryId = c.c.Id,
            ...
            //4. Sort
            query2 = query2.OrderByDynamic<ProvinceViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<ProvinceViewModel>()
            {

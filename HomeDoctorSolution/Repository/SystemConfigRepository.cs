...
namespace HomeDoctorSolution.Repository
{
    public class SystemConfigRepository : ISystemConfigRepository
    {
        ...
        public SystemConfigRepository(HomeDoctorContext _db)
        {
            db = _db;
        }
        ...
        public async Task<SystemConfig> Detail(int? id)
        {
            if (db != null)
            {
                return await db.SystemConfigs.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
            }
            return null;
        }
        ...
        public async Task<SystemConfig> Add(SystemConfig obj)
        {
            if (db != null)
            {
                await db.SystemConfigs.AddAsync(obj);
        ...
        public async Task<int> DeletePermanently(int? objId)
        {
            ...
            if (db != null)
            {
                ...
                var obj = await db.SystemConfigs.FirstOrDefaultAsync(x => x.Id == objId);
        ...
        public async Task<DTResult<SystemConfig>> ListServerSide(SystemConfigDTParameters parameters)
        {
            ...
            //1. Join
            var query = from row in db.SystemConfigs
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
            ...
            //3.Query second
            var query2 = query.Select(c => new SystemConfig()
            {
                Id = c.row.Id,
                Active = c.row.Active,
                Name = c.row.Name,
            ...
            //4. Sort
            query2 = query2.OrderByDynamic<SystemConfig>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<SystemConfig>()
            {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class SurveyTypeRepository: ISurveyTypeRepository
                {
                    ...
                    public SurveyTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.SurveyTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.SurveyTypes.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.SurveyTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.SurveyTypes.Remove(obj);
            ...
            public async Task <DTResult<SurveyType>> ListServerSide(SurveyTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.SurveyTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new SurveyType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<SurveyType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<SurveyType>()
                {

...
namespace HomeDoctorSolution.Repository
{
    public class ProductStatusRepository : IProductStatusRepository
    {
        ...
        public ProductStatusRepository(HomeDoctorContext _db)
        {
            db = _db;
        }
        ...
        public async Task<List<ProductStatus>> List()
        {
            ...
            {
                return await (
                    from row in db.ProductStatuses
                    where (row.Active == 1)
        ...
        public async Task<ProductStatus> Detail(int? id)
        {
            if (db != null)
            {
                return await db.ProductStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
            }
            return null;
        }
        ...
        public async Task<ProductStatus> Add(ProductStatus obj)
        {
            if (db != null)
            {
                await db.ProductStatuses.AddAsync(obj);
        ...
        public async Task<int> DeletePermanently(int? objId)
        {
            ...
            if (db != null)
            {
                ...
                var obj = await db.ProductStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                if (obj != null)
                {
                    //Delete that obj
                    db.ProductStatuses.Remove(obj);
        ...
        public async Task<DTResult<ProductStatus>> ListServerSide(ProductStatusDTParameters parameters)
        {
            ...
            //1. Join
            var query = from row in db.ProductStatuses
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
            ...
            //3.Query second
            var query2 = query.Select(c => new ProductStatus()
            {
                Id = c.row.Id,
                Active = c.row.Active,
                Name = c.row.Name,
            ...
            //4. Sort
            query2 = query2.OrderByDynamic<ProductStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<ProductStatus>()
            {
        ...
        {
            return await db.ProductStatuses.AnyAsync(c => c.Active == 1 && c.Name.ToLower().Trim() == name.ToLower().Trim() && c.Id != id);
        }
    }
}

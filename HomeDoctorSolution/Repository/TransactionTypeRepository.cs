    ...
    namespace HomeDoctorSolution.Repository
        {
            public class TransactionTypeRepository: ITransactionTypeRepository
                {
                    ...
                    public TransactionTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<TransactionType>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.TransactionTypes
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.TransactionTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.TransactionTypes.AddAsync(obj);
            ...
            public async Task Update(TransactionType obj)
            {
                if (db != null) {
                    //Update that object
                    db.TransactionTypes.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.TransactionTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.TransactionTypes.Remove(obj);
            ...
            public async Task <DTResult<TransactionType>> ListServerSide(TransactionTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.TransactionTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new TransactionType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<TransactionType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<TransactionType>()
                {

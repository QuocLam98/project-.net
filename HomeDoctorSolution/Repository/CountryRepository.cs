    ...
    namespace HomeDoctorSolution.Repository
        {
            public class CountryRepository: ICountryRepository
                {
                    ...
                    public CountryRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.Countries.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.Countries.AddAsync(obj);
            ...
            public async Task Update(Country obj)
            {
                if (db != null) {
                    //Update that object
                    db.Countries.Attach(obj);
                    db.Entry(obj).Property(x => x.Active).IsModified = true;
db.Entry(obj).Property(x => x.Name).IsModified = true;
db.Entry(obj).Property(x => x.CountryCode).IsModified = true;
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.Countries.FirstOrDefaultAsync(x => x.Id == objId);
            ...
            public async Task <DTResult<Country>> ListServerSide(CountryDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.Countries 
...
EF.Functions.Collate(c.row.CountryCode.ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General)) ||
...
case "active":
 ...
 case "name":
                ...
                break;
 case "countryCode":
                query = query.Where(c => (c.row.CountryCode ?? "").Contains(fillter));
                break;
 case "description":
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new Country()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
CountryCode = c.row.CountryCode,
Description = c.row.Description,
CreatedTime = c.row.CreatedTime,

                });
                //4. Sort
                query2 = query2.OrderByDynamic<Country>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<Country>()
                {

...
namespace HomeDoctorSolution.Repository
{
    public class FavouritePostRepository : IFavouritePostRepository
    {
        ...
        public FavouritePostRepository(HomeDoctorContext _db)
        {
            db = _db;
        }
        ...
        public async Task<FavouritePost> Detail(int? id)
        {
            if (db != null)
            {
                return await db.FavouritePosts.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
            }
            return null;
        }
        ...
        public async Task<FavouritePost> Add(FavouritePost obj)
        {
            if (db != null)
            {
                await db.FavouritePosts.AddAsync(obj);
        ...
        public async Task<int> DeletePermanently(int? objId)
        {
            ...
            if (db != null)
            {
                ...
                var obj = await db.FavouritePosts.FirstOrDefaultAsync(x => x.Id == objId);

                if (obj != null)
                {
                    //Delete that obj
                    db.FavouritePosts.Remove(obj);
        ...
        public async Task<DTResult<FavouritePostViewModel>> ListServerSide(FavouritePostDTParameters parameters)
        {
            ...
            //1. Join
            var query = from row in db.FavouritePosts
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
            ...
            //3.Query second
            var query2 = query.Select(c => new FavouritePostViewModel()
            {
                Id = c.row.Id,
                PostId = c.p.Id,
                PostName = c.p.Name,
            ...
            //4. Sort
            query2 = query2.OrderByDynamic<FavouritePostViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<FavouritePostViewModel>()
            {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class SurveySectionQuestionRepository: ISurveySectionQuestionRepository
                {
                    ...
                    public SurveySectionQuestionRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<SurveySectionQuestion>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.SurveySectionQuestions
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.SurveySectionQuestions.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.SurveySectionQuestions.AddAsync(obj);
            ...
            public async Task Update(SurveySectionQuestion obj)
            {
                if (db != null) {
                    //Update that object
                    db.SurveySectionQuestions.Attach(obj);
...
db.Entry(obj).Property(x => x.Text).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.SurveySectionQuestions.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.SurveySectionQuestions.Remove(obj);
            ...
            public async Task <DTResult<SurveySectionQuestionViewModel>> ListServerSide(SurveySectionQuestionDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.SurveySectionQuestions 

                                    join ss in db.SurveySections on row.SurveySectionId equals ss.Id 
...
case "createdTime":
                ...
                break;
                                ...
                                if (parameters.SurveySectionIds.Count > 0)
                                {
                                    query = query.Where(c => parameters.SurveySectionIds.Contains(c.row.SurveySection.Id));
                                }
                ...
                //3.Query second
                var query2 = query.Select(c => new SurveySectionQuestionViewModel()
                {
                    Id = c.row.Id,
SurveySectionId = c.ss.Id,
SurveySectionName = c.ss.Name,
QuestionId = c.q.Id,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<SurveySectionQuestionViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<SurveySectionQuestionViewModel>()
                {

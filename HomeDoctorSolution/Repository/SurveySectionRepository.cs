    ...
    namespace HomeDoctorSolution.Repository
        {
            public class SurveySectionRepository: ISurveySectionRepository
                {
                    ...
                    public SurveySectionRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.SurveySections.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.SurveySections.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Text).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.SurveySections.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.SurveySections.Remove(obj);
            ...
            public async Task <DTResult<SurveySectionViewModel>> ListServerSide(SurveySectionDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.SurveySections 
...
EF.Functions.Collate(c.row.ProportionScore.ToString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General)) ||
...
case "active":
                        ...
                        break;
case "proportionScore":
                        query = query.Where(c => c.row.ProportionScore.ToString().Trim().Contains(fillter));
                        break;
 case "name":
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new SurveySectionViewModel()
                {
                    Id = c.row.Id,
SurveyId = c.s.Id,
SurveyName = c.s.Name,
...
Active = c.row.Active,
ProportionScore = c.row.ProportionScore,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<SurveySectionViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<SurveySectionViewModel>()
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class DoctorTypeRepository: IDoctorTypeRepository
                {
                    ...
                    public DoctorTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.DoctorTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <DoctorType> Add(DoctorType obj)
            {
                if (db != null) {
                    await db.DoctorTypes.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.DoctorTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.DoctorTypes.Remove(obj);
            ...
            public async Task <DTResult<DoctorType>> ListServerSide(DoctorTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.DoctorTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new DoctorType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<DoctorType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<DoctorType>()
                {

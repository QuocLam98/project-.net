    ...
    namespace HomeDoctorSolution.Repository
        {
            public class SurveySectionAccountDetailRepository: ISurveySectionAccountDetailRepository
                {
                    ...
                    public SurveySectionAccountDetailRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<SurveySectionAccountDetail>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.SurveySectionAccountDetails
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.SurveySectionAccountDetails.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.SurveySectionAccountDetails.AddAsync(obj);
            ...
            public async Task Update(SurveySectionAccountDetail obj)
            {
                if (db != null) {
                    //Update that object
                    db.SurveySectionAccountDetails.Attach(obj);
                    db.Entry(obj).Property(x => x.Active).IsModified = true;
db.Entry(obj).Property(x => x.SurveySectionAccountId).IsModified = true;
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.SurveySectionAccountDetails.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.SurveySectionAccountDetails.Remove(obj);
            ...
            public async Task <DTResult<SurveySectionAccountDetailViewModel>> ListServerSide(SurveySectionAccountDetailDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.SurveySectionAccountDetails 

                                    join ssa in db.SurveySectionAccounts on row.SurveySectionAccountId equals ssa.Id 
join q in db.Questions on row.QuestionId equals q.Id 
join a in db.Answers on row.AnswerId equals a.Id 
...
case "createdTime":
                ...
                break;
                                ...
                                if (parameters.SurveySectionAccountIds.Count > 0)
                                {
                                    query = query.Where(c => parameters.SurveySectionAccountIds.Contains(c.row.SurveySectionAccountId));
                                }
                                

                                if (parameters.QuestionIds.Count > 0)
                                {
                                    query = query.Where(c => parameters.QuestionIds.Contains(c.row.Question.Id));
                                }
                                

                                if (parameters.AnswerIds.Count > 0)
                                {
                                    query = query.Where(c => parameters.AnswerIds.Contains(c.row.AnswerId));
                                }
                                

                //3.Query second
                var query2 = query.Select(c => new SurveySectionAccountDetailViewModel()
                {
                    Id = c.row.Id,
Active = c.row.Active,
SurveySectionAccountId = c.ssa.Id,
SurveySectionAccountName = c.ssa.Name,
QuestionId = c.q.Id,
QuestionName = c.q.Name,
AnswerId = c.a.Id,
AnswerName = c.a.Name,
Score = c.row.Score,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<SurveySectionAccountDetailViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<SurveySectionAccountDetailViewModel>()
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class OrganizationStatusRepository: IOrganizationStatusRepository
                {
                    HomeDoctorContext db;
                    public OrganizationStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<OrganizationStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.OrganizationStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.OrganizationStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <OrganizationStatus> Add(OrganizationStatus obj)
            {
                if (db != null) {
                    await db.OrganizationStatuses.AddAsync(obj);
            ...
            public async Task Update(OrganizationStatus obj)
            {
                if (db != null) {
                    //Update that object
                    db.OrganizationStatuses.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.OrganizationStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.OrganizationStatuses.Remove(obj);
            ...
            public async Task <DTResult<OrganizationStatus>> ListServerSide(OrganizationStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.OrganizationStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new OrganizationStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<OrganizationStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<OrganizationStatus>()
                {

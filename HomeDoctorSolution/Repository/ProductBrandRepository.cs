    ...
    namespace HomeDoctorSolution.Repository
        {
            public class ProductBrandRepository: IProductBrandRepository
                {
                    ...
                    public ProductBrandRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.ProductBrands.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <ProductBrand> Add(ProductBrand obj)
            {
                if (db != null) {
                    await db.ProductBrands.AddAsync(obj);

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class ActivityLogRepository: IActivityLogRepository
                {
                    ...
                    public ActivityLogRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.ActivityLogs.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
...
EF.Functions.Collate(c.row.GuId.ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General)) ||

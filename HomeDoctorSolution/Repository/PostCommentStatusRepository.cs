    ...
    namespace HomeDoctorSolution.Repository
        {
            public class PostCommentStatusRepository: IPostCommentStatusRepository
                {
                    HomeDoctorContext db;
                    public PostCommentStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<PostCommentStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.PostCommentStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.PostCommentStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <PostCommentStatus> Add(PostCommentStatus obj)
            {
                if (db != null) {
                    await db.PostCommentStatuses.AddAsync(obj);
            ...
            public async Task Update(PostCommentStatus obj)
            {
                if (db != null) {
                    //Update that object
                    db.PostCommentStatuses.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.PostCommentStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.PostCommentStatuses.Remove(obj);
            ...
            public async Task <DTResult<PostCommentStatus>> ListServerSide(PostCommentStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.PostCommentStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new PostCommentStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<PostCommentStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<PostCommentStatus>()
                {

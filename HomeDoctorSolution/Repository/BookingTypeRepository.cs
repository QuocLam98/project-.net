    ...
    namespace HomeDoctorSolution.Repository
        {
            public class BookingTypeRepository: IBookingTypeRepository
                {
                    ...
                    public BookingTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.BookingTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <BookingType> Add(BookingType obj)
            {
                if (db != null) {
                    await db.BookingTypes.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.BookingTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.BookingTypes.Remove(obj);
            ...
            public async Task <DTResult<BookingType>> ListServerSide(BookingTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.BookingTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new BookingType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<BookingType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<BookingType>()
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class PostMetaRepository: IPostMetaRepository
                {
                    ...
                    public PostMetaRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.PostMeta.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task <DTResult<PostMetaViewModel>> ListServerSide(PostMetaDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.PostMeta 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new PostMetaViewModel()
                {
                    Id = c.row.Id,
PostId = c.p.Id,
PostName = c.p.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<PostMetaViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<PostMetaViewModel>()
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class AccountTypeRepository: IAccountTypeRepository
                {
                    ...
                    public AccountTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.AccountTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.AccountTypes.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.AccountTypes.FirstOrDefaultAsync(x => x.Id == objId);
            ...
            public async Task <DTResult<AccountType>> ListServerSide(AccountTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.AccountTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new AccountType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<AccountType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<AccountType>()
                {

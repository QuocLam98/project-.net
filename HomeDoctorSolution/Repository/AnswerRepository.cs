    ...
    namespace HomeDoctorSolution.Repository
        {
            public class AnswerRepository: IAnswerRepository
                {
                    ...
                    public AnswerRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.Answers.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.Answers.FirstOrDefaultAsync(x => x.Id == objId);
            ...
            public async Task <DTResult<AnswerViewModel>> ListServerSide(AnswerDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.Answers 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new AnswerViewModel()
                {
                    Id = c.row.Id,
QuestionId = c.q.Id,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<AnswerViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<AnswerViewModel>()
                {

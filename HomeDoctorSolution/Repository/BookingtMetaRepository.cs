    ...
    namespace HomeDoctorSolution.Repository
        {
            public class BookingtMetaRepository: IBookingtMetaRepository
                {
                    HomeDoctorContext db;
                    public BookingtMetaRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.BookingtMeta.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <BookingtMeta> Add(BookingtMeta obj)
            {
                if (db != null) {
                    await db.BookingtMeta.AddAsync(obj);
            ...
            public async Task Update(BookingtMeta obj)
            {
                if (db != null) {
                    //Update that object
                    db.BookingtMeta.Attach(obj);
                    db.Entry(obj).Property(x => x.BookingId).IsModified = true;
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.BookingtMeta.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.BookingtMeta.Remove(obj);
            ...
            public async Task <DTResult<BookingtMetaViewModel>> ListServerSide(BookingtMetaDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.BookingtMeta 

                                    join b in db.Bookings on row.BookingId equals b.Id 
...
case "createdTime":
                ...
                break;
                                ...
                                if (parameters.BookingIds.Count > 0)
                                {
                                    query = query.Where(c => parameters.BookingIds.Contains(c.row.Booking.Id));
                                }
                                

                //3.Query second
                var query2 = query.Select(c => new BookingtMetaViewModel()
                {
                    Id = c.row.Id,
BookingId = c.b.Id,
BookingName = c.b.Name,
Active = c.row.Active,
Name = c.row.Name,
Key = c.row.Key,
Value = c.row.Value,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<BookingtMetaViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<BookingtMetaViewModel>()
                {

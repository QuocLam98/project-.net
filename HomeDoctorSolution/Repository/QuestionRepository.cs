    ...
    namespace HomeDoctorSolution.Repository
        {
            public class QuestionRepository: IQuestionRepository
                {
                    ...
                    public QuestionRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.Questions.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
...
db.Entry(obj).Property(x => x.Text).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.Questions.FirstOrDefaultAsync(x => x.Id == objId);
            ...
            public async Task <DTResult<QuestionViewModel>> ListServerSide(QuestionDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.Questions 

                                    join qt in db.QuestionTypes on row.QuestionTypeId equals qt.Id 
...
case "createdTime":
                ...
                break;
                                ...
                                if (parameters.QuestionTypeIds.Count > 0)
                                {
                                    query = query.Where(c => parameters.QuestionTypeIds.Contains(c.row.QuestionType.Id));
                                }
                                

                //3.Query second
                var query2 = query.Select(c => new QuestionViewModel()
                {
                    Id = c.row.Id,
QuestionTypeId = c.qt.Id,
QuestionTypeName = c.qt.Name,
Active = c.row.Active,
Photo = c.row.Photo,
Name = c.row.Name,
Score = c.row.Score,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<QuestionViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<QuestionViewModel>()
                {

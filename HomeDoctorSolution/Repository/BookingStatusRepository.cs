    ...
    namespace HomeDoctorSolution.Repository
        {
            public class BookingStatusRepository: IBookingStatusRepository
                {
                    ...
                    public BookingStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<BookingStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.BookingStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.BookingStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <BookingStatus> Add(BookingStatus obj)
            {
                if (db != null) {
                    await db.BookingStatuses.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.BookingStatuses.FirstOrDefaultAsync(x => x.Id == objId);
            ...
            public async Task <DTResult<BookingStatus>> ListServerSide(BookingStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.BookingStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new BookingStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<BookingStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<BookingStatus>()
                {

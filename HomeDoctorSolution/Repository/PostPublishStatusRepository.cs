    ...
    namespace HomeDoctorSolution.Repository
        {
            public class PostPublishStatusRepository: IPostPublishStatusRepository
                {
                    HomeDoctorContext db;
                    public PostPublishStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<PostPublishStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.PostPublishStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.PostPublishStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <PostPublishStatus> Add(PostPublishStatus obj)
            {
                if (db != null) {
                    await db.PostPublishStatuses.AddAsync(obj);
            ...
            public async Task Update(PostPublishStatus obj)
            {
                if (db != null) {
                    //Update that object
                    db.PostPublishStatuses.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.PostPublishStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.PostPublishStatuses.Remove(obj);
            ...
            public async Task <DTResult<PostPublishStatus>> ListServerSide(PostPublishStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.PostPublishStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new PostPublishStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<PostPublishStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<PostPublishStatus>()
                {

...
namespace HomeDoctorSolution.Repository
{
    public class ReadedPostRepository : IReadedPostRepository
    {
        ...
        public ReadedPostRepository(HomeDoctorContext _db)
        {
            db = _db;
        }
        ...
        public async Task<ReadedPost> Detail(int? id)
        {
            if (db != null)
            {
                return await db.ReadedPosts.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
            }
            return null;
        }
        ...
        public async Task<ReadedPost> Add(ReadedPost obj)
        {
            if (db != null)
            {
                await db.ReadedPosts.AddAsync(obj);
        ...
        public async Task<int> DeletePermanently(int? objId)
        {
            ...
            if (db != null)
            {
                ...
                var obj = await db.ReadedPosts.FirstOrDefaultAsync(x => x.Id == objId);

                if (obj != null)
                {
                    //Delete that obj
                    db.ReadedPosts.Remove(obj);
        ...
        public async Task<DTResult<ReadedPostViewModel>> ListServerSide(ReadedPostDTParameters parameters)
        {
            ...
            //1. Join
            var query = from row in db.ReadedPosts
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
            ...
            //3.Query second
            var query2 = query.Select(c => new ReadedPostViewModel()
            {
                Id = c.row.Id,
                PostId = c.p.Id,
                PostName = c.p.Name,
            ...
            //4. Sort
            query2 = query2.OrderByDynamic<ReadedPostViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<ReadedPostViewModel>()
            {

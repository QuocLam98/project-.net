    ...
    namespace HomeDoctorSolution.Repository
        {
            public class OrganizationTypeRepository: IOrganizationTypeRepository
                {
                    ...
                    public OrganizationTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<OrganizationType>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.OrganizationTypes
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.OrganizationTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <OrganizationType> Add(OrganizationType obj)
            {
                if (db != null) {
                    await db.OrganizationTypes.AddAsync(obj);
            ...
            public async Task Update(OrganizationType obj)
            {
                if (db != null) {
                    //Update that object
                    db.OrganizationTypes.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.OrganizationTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.OrganizationTypes.Remove(obj);
            ...
            public async Task <DTResult<OrganizationType>> ListServerSide(OrganizationTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.OrganizationTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new OrganizationType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<OrganizationType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<OrganizationType>()
                {

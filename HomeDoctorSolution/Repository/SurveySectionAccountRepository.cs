    ...
    namespace HomeDoctorSolution.Repository
        {
            public class SurveySectionAccountRepository: ISurveySectionAccountRepository
                {
                    ...
                    public SurveySectionAccountRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<SurveySectionAccount>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.SurveySectionAccounts
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.SurveySectionAccounts.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.SurveySectionAccounts.AddAsync(obj);
            ...
            public async Task Update(SurveySectionAccount obj)
            {
                if (db != null) {
                    //Update that object
                    db.SurveySectionAccounts.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.SurveySectionAccounts.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.SurveySectionAccounts.Remove(obj);
            ...
            public async Task <DTResult<SurveySectionAccountViewModel>> ListServerSide(SurveySectionAccountDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.SurveySectionAccounts 

                                    join sa in db.SurveyAccounts on row.SurveyAccountId equals sa.Id 
...
case "createdTime":
                ...
                break;
                                ...
                                if (parameters.SurveyAccountIds.Count > 0)
                                {
                                    query = query.Where(c => parameters.SurveyAccountIds.Contains(c.row.SurveyAccount.Id));
                                }
                                

                //3.Query second
                var query2 = query.Select(c => new SurveySectionAccountViewModel()
                {
                    Id = c.row.Id,
SurveyAccountId = c.sa.Id,
SurveyAccountName = c.sa.Name,
Active = c.row.Active,
Score = c.row.Score,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<SurveySectionAccountViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<SurveySectionAccountViewModel>()
                {

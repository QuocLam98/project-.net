...
namespace HomeDoctorSolution.Repository
{
    public class PostTagRepository : IPostTagRepository
    {
        ...
        public PostTagRepository(HomeDoctorContext _db)
        {
            db = _db;
        }
        ...
        public async Task<PostTag> Detail(int? id)
        {
            if (db != null)
            {
                return await db.PostTags.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
            }
            return null;
        }
        ...
        public async Task<int> DeletePermanently(int? objId)
        {
            ...
            if (db != null)
            {
                ...
                var obj = await db.PostTags.FirstOrDefaultAsync(x => x.Id == objId);
        ...
        public async Task<DTResult<PostTagViewModel>> ListServerSide(PostTagDTParameters parameters)
        {
            ...
            //1. Join
            var query = from row in db.PostTags

                        join p in db.Posts on row.PostId equals p.Id
                        join t in db.Tags on row.TagId equals t.Id
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
            ...
            if (parameters.TagIds.Count > 0)
            {
                query = query.Where(c => parameters.TagIds.Contains(c.row.Tag.Id));
            }


            //3.Query second
            var query2 = query.Select(c => new PostTagViewModel()
            {
                Id = c.row.Id,
                PostId = c.p.Id,
                PostName = c.p.Name,
            ...
            //4. Sort
            query2 = query2.OrderByDynamic<PostTagViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<PostTagViewModel>()
            {
        ...
        public async Task InsertManyAsync(IEnumerable<PostTag> objs)
        {
            await db.PostTags.AddRangeAsync(objs);
            await db.SaveChangesAsync();
        }
    }
}

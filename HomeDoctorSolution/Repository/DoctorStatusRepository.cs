    ...
    namespace HomeDoctorSolution.Repository
        {
            public class DoctorStatusRepository: IDoctorStatusRepository
                {
                    HomeDoctorContext db;
                    public DoctorStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.DoctorStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <DoctorStatus> Add(DoctorStatus obj)
            {
                if (db != null) {
                    await db.DoctorStatuses.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.DoctorStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.DoctorStatuses.Remove(obj);
            ...
            public async Task <DTResult<DoctorStatus>> ListServerSide(DoctorStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.DoctorStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new DoctorStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<DoctorStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<DoctorStatus>()
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class PostCategoryRepository: IPostCategoryRepository
                {
                    ...
                    public PostCategoryRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.PostCategories.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.PostCategories.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Color).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.PostCategories.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.PostCategories.Remove(obj);
            ...
            public async Task <DTResult<PostCategory>> ListServerSide(PostCategoryDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.PostCategories 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new PostCategory()
                {
                    Id = c.row.Id,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<PostCategory>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<PostCategory>()
                {

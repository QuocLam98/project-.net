    ...
    namespace HomeDoctorSolution.Repository
        {
            public class CommentStatusRepository: ICommentStatusRepository
                {
                    HomeDoctorContext db;
                    public CommentStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<CommentStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.CommentStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.CommentStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <CommentStatus> Add(CommentStatus obj)
            {
                if (db != null) {
                    await db.CommentStatuses.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.CommentStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.CommentStatuses.Remove(obj);
            ...
            public async Task <DTResult<CommentStatus>> ListServerSide(CommentStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.CommentStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new CommentStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<CommentStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<CommentStatus>()
                {

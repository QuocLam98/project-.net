...
namespace HomeDoctorSolution.Repository
{
    public class ProductTypeRepository : IProductTypeRepository
    {
        ...
        public ProductTypeRepository(HomeDoctorContext _db)
        {
            db = _db;
        }
        ...
        public async Task<ProductType> Detail(int? id)
        {
            if (db != null)
            {
                return await db.ProductTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
            }
            return null;
        }


        public async Task<ProductType> Add(ProductType obj)
        {
            if (db != null)
            {
                await db.ProductTypes.AddAsync(obj);
        ...
        public async Task<int> DeletePermanently(int? objId)
        {
            ...
            if (db != null)
            {
                ...
                var obj = await db.ProductTypes.FirstOrDefaultAsync(x => x.Id == objId);
        ...
        public async Task<DTResult<ProductType>> ListServerSide(ProductTypeDTParameters parameters)
        {
            ...
            //1. Join
            var query = from row in db.ProductTypes
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
            ...
            //3.Query second
            var query2 = query.Select(c => new ProductType()
            {
                Id = c.row.Id,
                Active = c.row.Active,
                Name = c.row.Name,
            ...
            //4. Sort
            query2 = query2.OrderByDynamic<ProductType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<ProductType>()
            {
        ...
        {
            return await db.ProductTypes.AnyAsync(c => c.Active == 1 && c.Name.ToLower().Trim() == name.ToLower().Trim() && c.Id != id);
        }
    }
}

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class HomepageContentMetaRepository: IHomepageContentMetaRepository
                {
                    HomeDoctorContext db;
                    public HomepageContentMetaRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<HomepageContentMeta>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.HomepageContentMeta
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.HomepageContentMeta.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <HomepageContentMeta> Add(HomepageContentMeta obj)
            {
                if (db != null) {
                    await db.HomepageContentMeta.AddAsync(obj);
            ...
            public async Task Update(HomepageContentMeta obj)
            {
                if (db != null) {
                    //Update that object
                    db.HomepageContentMeta.Attach(obj);
                    db.Entry(obj).Property(x => x.HomepageContentId).IsModified = true;
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.HomepageContentMeta.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.HomepageContentMeta.Remove(obj);
            ...
            public async Task <DTResult<HomepageContentMetaViewModel>> ListServerSide(HomepageContentMetaDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.HomepageContentMeta 

                                    join hc in db.HomepageContents on row.HomepageContentId equals hc.Id 
...
case "createdTime":
                ...
                break;
                                ...
                                if (parameters.HomepageContentIds.Count > 0)
                                {
                                    query = query.Where(c => parameters.HomepageContentIds.Contains(c.row.HomepageContent.Id));
                                }
                                

                //3.Query second
                var query2 = query.Select(c => new HomepageContentMetaViewModel()
                {
                    Id = c.row.Id,
HomepageContentId = c.hc.Id,
HomepageContentName = c.hc.Name,
Active = c.row.Active,
Name = c.row.Name,
Key = c.row.Key,
Value = c.row.Value,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<HomepageContentMetaViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<HomepageContentMetaViewModel>()
                {

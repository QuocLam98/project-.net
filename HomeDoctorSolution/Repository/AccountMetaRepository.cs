...
namespace HomeDoctorSolution.Repository
{
    public class AccountMetaRepository : IAccountMetaRepository
    {
        ...
        public AccountMetaRepository(HomeDoctorContext _db)
        {
            db = _db;
        }
        ...
        public async Task<AccountMeta> Detail(int? id)
        {
            if (db != null)
            {
                return await db.AccountMeta.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
            }
            return null;
        }
        ...
        public async Task<DTResult<AccountMetaViewModel>> ListServerSide(AccountMetaDTParameters parameters)
        {
            ...
            //1. Join
            var query = from row in db.AccountMeta
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
            ...
            //3.Query second
            var query2 = query.Select(c => new AccountMetaViewModel()
            {
                Id = c.row.Id,
                AccountId = c.a.Id,
            ...
            //4. Sort
            query2 = query2.OrderByDynamic<AccountMetaViewModel>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<AccountMetaViewModel>()
            {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<AccountMeta> GetAccountMeta(string key, string hash)
        {
            return await (from acc in db.AccountMeta where (acc.Value == key && acc.Description == hash && acc.Active == 1) select acc).FirstOrDefaultAsync();
        }
        public async Task<AccountMeta> GetAccountMetaByValue(int accountId, string value)
        {
            return await (from acc in db.AccountMeta where (acc.Value == value && acc.AccountId == accountId && acc.Active == 1) select acc).FirstOrDefaultAsync();
        }
        public async Task DeActiveMeta(AccountMeta obj)
        {
            var acc = await db.AccountMeta.FirstAsync(x => x.Id == obj.Id);
            acc.Active = 0;
            //acc.Password = SecurityUtil.HashPassword(obj.Password.Trim());
            // thay doi 1 truong
            db.Attach(acc);
            db.Entry(acc).Property(s => s.Active).IsModified = true;

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class SubscribeRepository: ISubscribeRepository
                {
                    ...
                    public SubscribeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            {
                if (db != null) {
                    return await db.Subscribes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }
            ...
            {
                if (db != null) {
                    await db.Subscribes.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.Subscribes.FirstOrDefaultAsync(x => x.Id == objId);
            ...
            public async Task <DTResult<Subscribe>> ListServerSide(SubscribeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.Subscribes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new Subscribe()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<Subscribe>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<Subscribe>()
                {

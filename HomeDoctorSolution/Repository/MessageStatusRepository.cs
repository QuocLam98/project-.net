    ...
    namespace HomeDoctorSolution.Repository
        {
            public class MessageStatusRepository: IMessageStatusRepository
                {
                    ...
                    public MessageStatusRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<MessageStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.MessageStatuses
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.MessageStatuses.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <MessageStatus> Add(MessageStatus obj)
            {
                if (db != null) {
                    await db.MessageStatuses.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.MessageStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.MessageStatuses.Remove(obj);
            ...
            public async Task <DTResult<MessageStatus>> ListServerSide(MessageStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.MessageStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new MessageStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<MessageStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<MessageStatus>()
                {

    ...
    namespace HomeDoctorSolution.Repository
        {
            public class HomepageContentTypeRepository: IHomepageContentTypeRepository
                {
                    HomeDoctorContext db;
                    public HomepageContentTypeRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<HomepageContentType>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.HomepageContentTypes
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.HomepageContentTypes.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <HomepageContentType> Add(HomepageContentType obj)
            {
                if (db != null) {
                    await db.HomepageContentTypes.AddAsync(obj);
            ...
            public async Task Update(HomepageContentType obj)
            {
                if (db != null) {
                    //Update that object
                    db.HomepageContentTypes.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.HomepageContentTypes.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.HomepageContentTypes.Remove(obj);
            ...
            public async Task <DTResult<HomepageContentType>> ListServerSide(HomepageContentTypeDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.HomepageContentTypes 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new HomepageContentType()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<HomepageContentType>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<HomepageContentType>()
                {

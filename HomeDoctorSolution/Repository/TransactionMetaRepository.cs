    ...
    namespace HomeDoctorSolution.Repository
        {
            public class TransactionMetaRepository: ITransactionMetaRepository
                {
                    HomeDoctorContext db;
                    public TransactionMetaRepository(HomeDoctorContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<TransactionMeta>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.TransactionMeta
                        where(row.Active == 1)
            ...
            {
                if (db != null) {
                    return await db.TransactionMeta.FirstOrDefaultAsync(row => row.Active == 1 && row.Id == id);
                }
                return null;
            }


            public async Task <TransactionMeta> Add(TransactionMeta obj)
            {
                if (db != null) {
                    await db.TransactionMeta.AddAsync(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.TransactionMeta.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.TransactionMeta.Remove(obj);
            ...
            public async Task <DTResult<TransactionMeta>> ListServerSide(TransactionMetaDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.TransactionMeta 
...
EF.Functions.Collate(c.row.TransactionId.ToString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General)) ||
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
                ...
                foreach(var item in parameters.Columns)
                {
                    ...
                    if (fillter.Length > 0) {
                        ...
                        break;
case "transactionId":
                        query = query.Where(c => c.row.TransactionId.ToString().Trim().Contains(fillter));
                        break;
 case "Key":
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new TransactionMeta()
                {
                    Id = c.row.Id,
TransactionId = c.row.TransactionId,
Key = c.row.Key,
Value = c.row.Value,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<TransactionMeta>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<TransactionMeta>()
                {
